...
\textbf{Effect.} The mean inclusive closure ratio moved from $1.106$ to $1.001$ ($1.016$ once the truth-side correction below is included), and all
...
\textbf{Effect.}  Inclusive totals ($10^{-38}$~cm$^2$/Ar): FHC $1.06\pm0.27\to1.03\pm0.43$,
...
$13/14/27$ $\to$ per-run $30/23/53$ events, i.e.\ $1.8\%$ of the selected sample; purity
...
selection now fills the cut-flow histograms with the full weight; the cut-flow table in
the note keeps the tune-only yields with a caveat until the ntuples are reprocessed.
No unfolded result is affected.
...
EXT subtraction on fake data
  & Cancelled via \texttt{fake\_data\_ext\_cancel}, which is a fake-data convenience only.
    \textbf{It must be re-enabled before unblinding} and is listed as a required action in
    the note. \\
...
The closure $\chi^2$ values have $p$-values clustered near unity (median $0.95$ over the
...
\midrule
Mean inclusive closure ratio        & $1.106$, $1.001$ & now $1.016$ (truth-side correction) \\
