...
\captionof{table}{Integrated (unfolded) cross section $\sigma_\mathrm{int}$ over the
  ...
  \textbf{Caveat (under revision):} the RHC and Combined columns are affected by a
  per-run POT normalization bug that over-counted the multi-file RHC runs (Run~3 split
  across 5 ntuple files, Run~4 across 3), inflating the simulated background and biasing
  the extracted RHC/Combined cross sections low. The bug has been fixed (per-(run,sample)
  MC POT normalization); these columns are being recomputed. The FHC column is unaffected
  (one MC file per run). Consequently the ``RHC below FHC'' statement above is provisional
  until the corrected numbers are available.}
