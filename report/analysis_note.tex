...
\begin{titlepage}
  ...
  \begin{abstract}
    ...
    by more than $0.8\sigma$; an independent-generator closure is not yet available, the one existing
    NuWro overlay having failed validation against the standalone NuWro prediction.
...
\begin{itemize}\itemsep1pt \setlength{\parskip}{0pt}
  ...
  \item \textbf{Validated:} the implementation, the size of the statistical uncertainties, and
    ...
    \S\ref{sec:altmodel}). \textbf{Not yet available:} an independent-generator closure --- the one
    existing NuWro overlay could not be validated against the standalone NuWro prediction and is
    not used. Not tested for the proton-tagged family.
...
\begin{enumerate}\itemsep3pt
  ...
  \item \textbf{Robustness to model mismatch.} Unfold pseudo-data whose truth differs materially
    ...
    response then introduce. With \emph{reweighted} GENIE pseudo-data, \emph{passed with a caveat} (\S\ref{sec:altmodel}); with an \emph{independent generator}, \emph{not yet available} --- the one existing NuWro overlay could not be validated and is not used.
\end{enumerate}
...
\paragraph{Independent-generator closure: not yet available.} The stronger form of the third
test --- pseudo-data from a different generator with its own detector simulation --- was attempted
with the one NuWro overlay of Run-1 FHC that exists. It is not used. The sample's normalisation does
not agree with the standalone NuWro prediction for the same flux (its true signal rate per POT is
$1.76$ times GENIE's, against the $1.6$ implied by the standalone NuWro cross section and the raw
flux), and its provenance could not be established, so a closure against it would test the
sample rather than the extraction. A properly produced NuWro or GiBUU overlay remains the
outstanding validation item (\S\ref{sec:limitations}).
