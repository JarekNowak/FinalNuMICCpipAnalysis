...
overlaid with the GENIE and GiBUU predictions for the same proton-tagged phase space.
The generator predictions are built at truth level from the generator-level final state
(GENIE gst events, GiBUU \texttt{FinalEvents}) by applying the inclusive signal plus a
leading true proton above $0.3$~GeV/$c$ and evaluating the six observables with the same
formulas as the selection (\S\ref{sec:wtki_reco}), then combining the $\nu_\mu$ and
$\bar\nu_\mu$ samples over the FHC flux to a per-argon cross section. The two available
generators bracket the data: GiBUU predicts a larger integrated proton-tagged cross
section ($0.74$) than GENIE ($0.50\times10^{-38}$~cm$^2$/Ar), with the data in between,
and they differ most in the low-$\delta p_T$ / high-$p_n$ region sensitive to the proton
kinematics and final-state interactions. NuWro and NEUT are not overlaid: NuWro requires
reprocessing its event output inside its own container, and NEUT provides only tabulated
inclusive cross sections with no event-level kinematics. As throughout, the unfolded fake
data sit above the $A_C$-smeared truth by the Wiener-SVD normalisation factor.
...
\begin{figure}[H]
  ...
  \caption{Proton-tagged (\texttt{CC1mu1pi1p}) differential cross sections for FHC:
    ...
    MicroBooNE tune (black), and the GENIE (blue) and GiBUU (green dashed) predictions
    built for the same proton-tagged phase space. $W_{\pi p}$ falls from the
