...
{\footnotesize\ttfamily release 2026-09-04 \textperiodcentered\ 51 differential extractions $+$ 6 one-bin totals \textperiodcentered\ blind\par}
  ...
  \begin{abstract}
    ...
    realised truth to $0.975$--$0.99$; the RHC/FHC ratio of the injected tune is $0.901$. The Wiener-SVD integrated cross sections are of order
    ...
    framework on the resolution-optimised binning of \suppref{Sec.}{sec:binopt}.
      ...
      differently from the best-conditioned one (ensemble of the 2026-08-31 release,
      thrown with the earlier weight rule that dropped non-finite-weight events; a
      re-run with the unified rule is in progress).  Separately, the 2026-09-02 release rebuilt bit-identically
      from the processed ntuples with every cached universe deleted, and the present
      release was built the same way in a single pass, so the released numbers are
      regenerable from the raw inputs rather than from a preserved intermediate.  The same ensemble shows a $2.5\%$ negative offset
      of the mean; about $1.4\%$ of it is the throw rule of that pass, which dropped
      non-finite-weight events, so the residual bias is below one standard deviation of
      the ensemble mean and is stated rather than corrected until the re-run settles it.
      %
...
of \S\ref{sec:cr_protocol} depends on: CC$0\pi$ is a quantitative test of the $0\pi$
background (target-class purity $72\%$ of the neutrino MC, $65\%$ of the full stack)
whose extrapolation to the signal region carries a $31$--$34\%$ uncertainty, dominated
by the detector variations; $\pi^0$ has the smallest transfer uncertainty
($14$--$17\%$, of which most is the finite statistics of the detector-variation samples
rather than a detector effect, \S\ref{sec:cr_transfer}) but its target class as counted in Table~\ref{tab:cr_transfer},
$\nu_\mu$CC with a $\pi^0$, is only $46\%$ of the region's neutrino MC ($41\%$ of the
full stack), because the NC$\pi^0$ events the shower veto is also meant to remove are
counted under ``NC and $\nu_e$CC''; under the $50\%$ rule as written it is therefore
diagnostic, and it becomes eligible for a fitted correction only if the collaboration
ratifies the target class as CC$+$NC~$\pi^0$ (the definition in Table~\ref{tab:cr_defs}),
whose purity is to be tabulated with the next reprocessing; multi-$\pi$ is diagnostic
only ($T\simeq2.5$, target purity $47\%$); and cosmic validates the beam-off
normalisation and shape but, at $34$--$41\%$ beam-off purity, is not eligible for a
fitted correction.
...
signal loss would gain about $5$ events of statistical uncertainty on a sample of $1\,612$.
...
rather than a physical or acceptance effect. That is all it can establish here: with
...
throughout, confirming the chain.
...
consistent at $-0.10$ to $-0.25$.  Part of this is now understood: the ensemble was thrown
with the earlier throw macro, which skipped events with a non-finite central-value weight
(change log), so its pseudo-data sat $0.9\%$ below the prediction in the signal region,
$\approx1.4\%$ on the extracted cross section after background subtraction. The residual,
$\approx1\%$, is below one standard deviation of the ensemble mean. The ensemble is being
re-run with the unified weight rule for $p_\mu$, $\cos\theta_\mu$ and the one-bin total
(which has no regularisation to hide a bias behind); until then the offset is stated,
not corrected.
...
with $\chi^2\le0.7$ on four to seven degrees of freedom and $p\ge0.98$ throughout. Two features stand out. First, the
...
\captionof{table}{Integrated (unfolded) cross section $\sigma_\mathrm{int}$ over the
  ...
  (\suppref{Sec.}{sec:crosschecks}) recovers a flat, $20$--$29\%$ higher total, confirming
...
$\sim\!250$ combined, before the beam-off sample's own statistics are included);
...
\begin{table}[H]
  ...
  \caption{Transfer factor $T=N^{\mathrm{SR}}/N^{\mathrm{CR}}$ of each control region's
    ...
    the CC$0\pi$ region as a reference. The Monte-Carlo-statistical floor of the detector
    entries, estimated from two disjoint halves of the variation samples
    (\texttt{sb\_detvar\_split.C}), is given in the text. \releasestamp}
...
upper limit set by the variation-sample statistics rather than a measurement, and the
multi-$\pi$ term is noise (floor $260\%$ FHC). Any fitted correction under
...
\begin{enumerate}\itemsep2pt
  ...
  \item \textbf{Statistics.} Three quantities are computed for each region and beam
    ...
    $21$--$32\%$ prediction systematics of Table~\ref{tab:cr_syst}, this pull cannot
    fail a discrepancy smaller than about $40$--$60\%$; it is therefore reported
    \emph{together with} the pull against the statistical and detector terms alone, which
    is the sensitive quantity for the high-statistics regions. (b)~A shape $\chi^2$ in
  ...
  \item \textbf{Permitted responses and the rule for choosing between them.} For a region
    ...
    excursion), which is treated as a reconstruction issue, not a model correction: the
    permitted response is to document it, to exclude the affected run period from
    \emph{both} the control-region comparison and the measurement with its exposure
    removed from the normalisation, and to re-approve; no cut or binning may be changed;
    ...
    Table~\ref{tab:cr_transfer_unc}, or its Monte-Carlo-statistical floor where that is
    larger (\S\ref{sec:cr_transfer}), is added to it; (iii)~otherwise the
    ...
    model: a correction of class~(c) is fitted to both modes simultaneously and applied to
    both, and a class that passes in one mode and fails in the other is treated under
    (b), never corrected in one mode alone. The signal contamination of each region
    ($1\,124$ events in CC$0\pi$ and $662$ in $\pi^0$ for FHC, more than the $943$
    signal events of the signal region) is held at the central-value tune in any fit; a
    signal mismodelling of $x$ therefore biases a fitted target-class correction by about
    $0.1x$ (CC$0\pi$) and $0.2x$ ($\pi^0$), which is stated with the result. If more
    than one region is flagged, no independent corrections are combined without
    accounting for shared events and correlated systematic variations: in particular,
    multi-$\pi$ and $\pi^0$ share events (\S\ref{sec:cr_overlap}) and are treated
    jointly, or multi-$\pi$ is designated diagnostic only, which its transfer factor
    already argues for.
...
\captionof{table}{FHC (Runs 1, 2, 4, 5): bin-averaged fractional uncertainty (\%) by
  ...
  (\texttt{logs/systdump}, 2026-09-04 release). ``Prediction total'' excludes the data
...
of $\delta\Phi/\Phi\times(1+B/S)$. With $B/S=668/943$ (FHC, Table~\ref{tab:cutcount})
and a reconstructed-level flux term of $17.7\%$ this gives $30.2\%$, which is the one-bin
FHC value ($30.25\%$); RHC ($15\%\times(1+744/1015)=26\%$ against $27.7\%$) and the
combined configuration follow the same arithmetic. The differential flux terms of
...
\begin{enumerate}\itemsep1pt
  ...
  \item \texttt{univmake} builds, per observable and configuration, the true/reco
    ...
    cluster; the last one, 2026-09-03, SLURM 3205128, rebuilt all 51 from the re-thrown
    fake data in one array job).
...
release was itself built from empty caches in a single pass (SLURM 3205128), so no
preserved intermediate exists for it.
...
\paragraph{The extraction may carry a small negative bias.}
The ensemble of thirty-two throws puts the mean $2.5\%$ below the reference, $2.1\sigma$ on
the mean, with per-bin pull means consistently negative in both observables tested. About
$1.4\%$ of that is the throw rule of the pass it was run on, which dropped
non-finite-weight events; the residual is below one standard deviation. The ensemble is
being re-run with the unified rule, including the one-bin total; no correction or
uncertainty is assigned meanwhile.
...
Twenty, each recorded in the change log with before-and-after values.  The largest were an
...
in one pass from the re-thrown fake data (\S\ref{sec:pipeline}); the bit-identical
comparison has not yet been repeated on it and is a prerequisite for unblinding.
...
(\S\ref{sec:pipeline}).  The ensemble of throws covers two FHC observables; extending it
