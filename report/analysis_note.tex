...
\begin{titlepage}
  ...
  \begin{abstract}
    ...
    chain; the release is reproducible bit-identically from the processed ntuples. Both closure
    tests use the nominal model on both sides, so what they establish is that the machinery is
    correct under that model --- robustness to a materially different interaction model has not yet
    been demonstrated and is an outstanding prerequisite. Sec.~\ref{sec:status} is the one-page
    version of the analysis and the status of every product in it.
  \end{abstract}
  \vfill
  \tableofcontents
\end{titlepage}
...
\begin{itemize}\itemsep1pt \setlength{\parskip}{0pt}
  ...
  \item \textbf{Validated:} the implementation against the nominal model, and the size of the
    statistical uncertainties. \textbf{Not yet demonstrated:} robustness to a materially different
    interaction model (\S\ref{sec:validation}).
...
$d\sigma/dp_\mu$
  & Analysis definition final; result blinded
  & Nominal configured covariance including detector variations. \\
$d\sigma/d\cos\theta_\mu$ and $d\sigma/d\theta_\mu$
  & Analysis definition final; result blinded
  & Nominal configured covariance; discriminates between models. \\
$d\sigma/d\cos\theta_\pi$
  & Analysis definition final; result blinded
  & Nominal configured covariance. (No $\theta_\pi$ parameterisation is extracted; only the muon angle
    is released in both forms.) \\
$d\sigma/d\theta_{\mu\pi}$
  & Analysis definition final; result blinded
  & Nominal configured covariance; the best-conditioned shape observable. \\
$d\sigma/dp_\pi$, two-bin
  & Analysis definition final, coarse; result blinded
  & Nominal configured covariance, but a two-region partition rather than a spectrum: one
    near-threshold interval and one broad inclusive interval above
    $0.205$~GeV/$c$.  See below. \\
\midrule
\multicolumn{3}{l}{\emph{Secondary scope --- nominal configured covariance, secondary results}}\\
\midrule
Proton-tagged integrated cross section
  & Nominal configured covariance
...
\begin{enumerate}\itemsep3pt
  ...
  \item \textbf{Robustness to model mismatch.} Unfold pseudo-data whose truth differs materially
    from the model that built the response --- another generator, or a reweighting that moves the
    pion-momentum or angular shape --- and ask how much bias the regularisation and the response
    model then introduce. \emph{Not yet demonstrated.}
\end{enumerate}

The third is the one that bounds how much of the result is the data and how much is the model
assumed in unfolding it, and its absence is the honest limit of what follows: the statements below
establish that the implementation is correct and its statistical uncertainties are the right size
\emph{under the nominal model}. They do not establish that the extraction is robust to the
underlying interaction model being wrong, and no claim in this note should be read that way. The
missing test is listed as a prerequisite in \S\ref{sec:limitations}.
