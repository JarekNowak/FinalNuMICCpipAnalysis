...
purity is $50.1\%$ (FHC) and $51.0\%$ (RHC) of the neutrino MC ($45$--$46\%$ of the
...
detector-variation samples (\S\ref{sec:cr_transfer}); multi-$\pi$ is diagnostic only
...
$0.89$--$0.95$ for the first bins of the angular observables, $0.67$--$0.88$ elsewhere in
...
low as $0.050$ so that the regularisation removes up to $95\%$ of a bin's normalisation;
...
$0.97$ and $1.02$, each determined to $\pm13\%$ with this many members --- and the
...
offset, not three. The earlier ensemble, on the 2026-08-31 release, showed $2.5\%$ at
...
\begin{table}[H]
  ...
  \caption{Transfer factor $T=N^{\mathrm{SR}}/N^{\mathrm{CR}}$ of each control region's
    ...
    Monte-Carlo statistics of the region itself. The ``all neutrino MC'' row uses the total
...
$12\%$ against $18\%$ in RHC), and the multi-$\pi$ term is noise (floor $260\%$ FHC). Any fitted correction under
...
uncertainties; that change is not made in this release.
