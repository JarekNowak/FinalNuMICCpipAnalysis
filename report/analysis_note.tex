...
\begin{itemize}\itemsep1pt
  ...
  \item[$\checkmark$] A control-region skim of the beam-on files, implemented
    ...
    file and a beam-off file processed both ways: the skim is exactly the ordered subset
...
fake-data truth using the \emph{full} systematic covariance, and the resulting $p$-values
...
2026-08-31 release; the current full-covariance values are those of
...
\begin{enumerate}\itemsep2pt
  ...
  \item \textbf{Acceptable agreement.} The control regions pass as a set if the global
    ...
    diagnostic: a region with $|{\rm pull}|>2$ (full covariance) or shape $p<0.05$ is
...
sparsest samples, where the unfolding amplifies them.  The central values were regenerated in the
same pass and therefore also postdate the central-value weighting fix.
...
Full systematic covariance propagation is complete for all fifty-one extractions.
...
Twenty-one result-affecting corrections, counting the $\cos\theta_\pi$ re-binning and
counting a bundled entry once, each recorded in the change log with before-and-after
