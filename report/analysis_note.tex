...
\begin{titlepage}
  ...
  \begin{abstract}
    ...
    complete configured covariance: flux, interaction model, hadron re-interaction, detector
    ...
    data; they show no evidence that the background model needs correcting.

    The extraction is validated by same-sample self-closure, by an ensemble of independent
    Poisson realisations carried through the whole chain --- which returns correctly sized
    statistical uncertainties --- and by an independent bit-identical rebuild from the processed
    ntuples. Sec.~\ref{sec:intro} explains how the note is organised; the studies behind the
    analysis choices are in the technical supplement, and the history of the analysis is in the
    change log.
  \end{abstract}
  \vfill
  \tableofcontents
...
\begin{enumerate}\itemsep3pt
  ...
  \item \textbf{Response and unfolding} (\S\ref{sec:unfolding}). The detector smears and loses
    ...
    it is compared with these results. $A_C$ is not normalisation-preserving, which is why
    integrated cross sections differ between observables of the same sample.
  ...
  \item \textbf{Validation and blinding} (\S\ref{sec:limitations}). The analysis is blind: results
    are extracted on fake data thrown from the central-value simulation. The background model is
    tested in four control regions that hold no signal-region event, which have been opened on
    beam-on data.
\end{enumerate}
...
proton-tagged subsample, which is secondary scope. Two companion documents carry material that
would otherwise interrupt the argument: the \emph{technical supplement} holds the studies behind
...
\begin{itemize}\itemsep3pt
  ...
  \item \textbf{One run's exposure is discrepant.} The global test passes in every period except
    FHC Run~1, where a free exposure factor fits $0.520\pm0.020$ with $\chi^2=16/17$. The size and
    the sharpness of that number point to a bookkeeping error in the run's exposure rather than to
    a mismodelled background; it is an open item (\S\ref{sec:limitations}), and no result here
    rests on FHC Run~1 alone.
...
The frozen $\pi^0$ definition drops the final multiplicity cut (\suppref{Sec.}{sec:request}). The
...
\begin{table}[H]
  ...
  \caption{The $\pi^0$ control region without (frozen) and with the final multiplicity
    ...
    class, the leverage denominator of \suppref{Sec.}{sec:request}; the CC$0\pi$ line is part of it.}
...
topologies the multi-shower argument of \suppref{Sec.}{sec:request} anticipated, and they are
...
\begin{enumerate}\itemsep2pt
  ...
  \item \textbf{Permitted responses and the rule for choosing between them.} The response
    ...
    requested in \suppref{Sec.}{sec:request}) and only if the
    ...
    (\suppref{Sec.}{sec:request}), so its eligibility is \emph{not} pre-authorised by this
    ...
    and $0.13$ in CC$0\pi$; \suppref{Sec.}{sec:request}).
    ...
    The response letters used here and in \suppref{Sec.}{sec:request} are (a)~no action,
...
The request of \suppref{Sec.}{sec:request} covers the four inclusive regions. The proton-tagged
