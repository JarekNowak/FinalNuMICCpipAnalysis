...
\begin{titlepage}
  ...
  \vspace{0.5cm}
  {\normalsize Draft 1.5, 2026-09-06\par}
\vspace{0.15cm}
{\footnotesize\ttfamily release 2026-09-06 \textperiodcentered\ 51 differential extractions $+$ 6 one-bin totals \textperiodcentered\ blind\par}
  ...
  \begin{abstract}
    ...
    framework on the resolution-optimised binning of \suppref{Sec.}{sec:binopt}.
      ...
      $W_{\pi p}$ is withdrawn; its systematics are otherwise complete.  All fifty-one
...
The proton-tagged results remain secondary scope for the narrower reason that their model
comparisons have not been recomputed since, and because the exposure does not support the
differential $W_{\pi p}$ shape at any binning.
...
\captionof{table}{Two-bin $p_\pi$ result as bin-integrated partial cross sections
  ...
  as density $\times$ physical width; the uncertainty is the data-statistical term of the
  release (the total fractional uncertainties are those of Tables~\ref{tab:fhc}--\ref{tab:comb}).
...
The row sums of $A_C$ are the relevant measure: a row summing to unity passes the
normalisation of that bin through unchanged, and a row summing to $0.2$ removes four
fifths of it.
...
The suppression is large: inclusive $\cos\theta_\mu$ retains as little as $5\%$ of a
bin's normalisation in RHC.  It also differs strongly between observables, which is
...
\begin{table}[H]
  ...
  \caption{Composition of the signal region (SR) and of each control region by background
    ...
    $\pi^0$ the $\pi^0$ class ($T\simeq0.02$; the ratified target class CC$+$NC~$\pi^0$,
  ...
  \begin{tabular}{lrrrrrcccc}
    ...
    \emph{CC$+$NC with a $\pi^0$ (ratified $\pi^0$ target; overlaps two rows above)} & 64.5 & 444 & 21.8 & 3\,331 & 3.2 & 0.15 & 2.96 & 0.019 & 20 \\
    \midrule
    ...
    \emph{CC$+$NC with a $\pi^0$ (ratified $\pi^0$ target; overlaps two rows above)} & 81.0 & 491 & 31.1 & 4\,118 & 3.6 & 0.17 & 2.61 & 0.020 & 22 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[H]
  ...
  \caption{Transfer factor $T=N^{\mathrm{SR}}/N^{\mathrm{CR}}$ of each control region's
    ...
    (\texttt{sb\_detvar\_split.C}), is given in the text; for the ratified CC$+$NC~$\pi^0$
  ...
  \begin{tabular}{lllccccccc}
    ...
    FHC & $\pi^0$ & CC$+$NC with $\pi^0$ (ratified) & $0.0194$ & $2.2$ & $5.8$ & $1.9$ & $4.2$ & $14.9$ ($12.1$) & $16.8$ \\
    \midrule
    ...
    RHC & $\pi^0$ & CC$+$NC with $\pi^0$ (ratified) & $0.0197$ & $1.7$ & $6.1$ & $1.6$ & $3.1$ & $11.8$ ($18.0$) & $19.4$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
holds for the ratified CC$+$NC~$\pi^0$ class: $15\%$ against a $12\%$ floor in FHC,
...
\begin{table}[H]
  ...
  \caption{FHC fake-data closure for the ten released proton-tagged observables.
    ...
    the $A_C$ smearing effect, not a bias. \releasestamp}
  \label{tab:wtki_fhc}
  \begin{tabular}{lccc}
\toprule
...
weakest part.}
Their systematics are complete and their central values current, but the exposure does not
...
totals of $41$--$47\%$.
Its systematics are complete, but the exposure does not support a differential $W_{\pi p}$
...
Twenty-one, counting the $\cos\theta_\pi$ re-binning, each recorded in the change log with
before-and-after values.  The largest were an
