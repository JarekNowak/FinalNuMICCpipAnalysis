...
\begin{titlepage}
  ...
  \begin{abstract}
    ...
    with the known over-prediction of $\mathrm{CC}\pi^+$ production in current
...
in the CC$\pi^+$ channel using the full NuMI exposure in FHC, RHC, and combined
...
at the finer six-bin binning. The generator overlays are \emph{not} shown in this version:
the GENIE, GiBUU, NEUT and NuWro proton-tagged W/TKI predictions were produced at the
earlier three-bin binning and are therefore dropped by the bin-count match against the
six-bin measurement. Regenerating them (GENIE and GiBUU natively, NuWro and NEUT from
their event vectors inside the SL7 container) at the current binning is pending; the
generator comparison quoted below refers to the earlier three-bin version. The predictions are built at truth level from each
...
over-prediction of CC$\pi^+$ production documented in
