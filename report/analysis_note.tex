...
It is \emph{not} uniform, and an earlier draft of this section overstated the case
by quoting $1.13$ for all three configurations. That figure is correct for $p_\pi$
alone, where the three agree to three decimal places; it is not representative of
the other five observables, and the claim is withdrawn.

It is not the additional smearing, since $A_C$ is applied to every prediction curve
and cancels in a comparison of unfolded data against $A_C$-smeared truth.
...
One further constraint narrows it usefully. The same-sample self-closure of
App.~\ref{sec:identity}, in which the \textsc{Genie} overlay is placed in the
\texttt{onBNB} slot so that data and response come from the \emph{same} sample,
recovers the truth to $0.94$--$1.06$ --- it does \emph{not} show this offset. The
offset therefore appears when the injected sample and the response differ (NuWro
fake data against a \textsc{Genie}-derived response) and vanishes when they do not,
which would point at model dependence in the unfolding rather than an algorithmic
bias.

That hypothesis has since been tested and is \emph{not} supported. It predicts that
the offset should grow with the NuWro/\textsc{Genie} disagreement in a given
observable. Across the eighteen extractions the correlation between the offset and
the NuWro/\textsc{Genie} integral ratio is $r=-0.32$: weak, and of the wrong sign,
since the observables where the two generators disagree most show the \emph{smaller}
offset. Whatever distinguishes the same-sample self-closure from the NuWro closure,
it is not simply the size of the generator disagreement.
...
This is not one awkward observable. Across the eighteen inclusive extractions the
row sums of $A_C$ --- what $\hat{x}=A_C\,x_\mathrm{true}$ predicts the ratio of
integrals should be --- span $0.24$ to $0.97$, a factor of $4.1$. The measured ratio
spans only $0.93$ to $1.23$, a factor of $1.3$, and shows no tendency to follow the
row sum. Fitting the two competing descriptions to all eighteen points:

\begin{center}\small
\begin{tabular}{lcc}
\toprule
Model & prediction & rms residual \\
\midrule
$\hat{x}=A_C\,x_\mathrm{true}$ (as documented) & ratio $=$ $A_C$ row sum & $0.503$ \\
$\hat{x}=k\,x_\mathrm{true}$ ($A_C$ irrelevant) & ratio $=$ const $=1.106$ & $0.081$ \\
\bottomrule
\end{tabular}
\end{center}

\noindent The constant-offset description is favoured by a factor of six in rms.
The unfolded normalisation is, empirically, \emph{independent of how strongly $A_C$
suppresses} --- $\cos\theta_\mu$ in RHC has a row sum of $0.24$ and still returns a
ratio of $0.98$. Either the $A_C$ written to the closure file is not the operator
actually applied to the data, or the unfolded histogram has already had its effect
divided out. Which of those it is decides whether the $11\%$ is a genuine
normalisation bias or an artefact of comparing two quantities that live in
different spaces, and it cannot be settled from the sidecar files alone. The
supporting numbers are in \texttt{report/offset\_vs\_AC.tsv}.
