...
single-shower events that already resemble the signal region. The size of that effect on
the region's yield, purity, transfer factor and overlap with multi-$\pi$ cannot be
quantified after the fact on the present processed files, which do not store the
multiplicities; it can be produced with a reprocessing of the Monte Carlo alone, which
touches no beam-on data, and that reprocessing is scheduled before the ratification of
item~(i) below. The frozen definition is the one in the table.

\paragraph{One point the collaboration is asked to ratify explicitly, and one already decided.}
(i)~The $\pi^0$ definition without the final multiplicity cut (Table~\ref{tab:cr_defs} and
the paragraph above): the comparison with the version retaining that cut is not available
in the current processed files and will \emph{not} be used to modify the region after
...
variations; the $\pi^0$ region's target class was ratified by the analysers on
...
full stack), so the region is eligible for a fitted correction under the $50\%$ rule,
narrowly, with transfer factor $T=0.019$--$0.020$ and a transfer uncertainty of
...
\begin{itemize}\itemsep1pt
  ...
  \item[$\checkmark$] A control-region skim of the beam-on files, implemented
    ...
    no run/subrun/event identifier, so no join-back by identifiers is possible; the file
  ...
  \item[$\square$] The $\pi^0$-definition comparison with and without the final
    multiplicity cut (\S\ref{sec:cr_pi0_final}), from the Monte-Carlo-only reprocess
    scheduled 2026-09-06, to be seen by the collaboration before item~(i) of
    \S\ref{sec:request} is ratified.
...
\begin{itemize}\itemsep1pt
  ...
  \item[$\square$] Documented treatment of any control-region discrepancy under
    \S\ref{sec:cr_protocol}, with renewed approval where item~4(d) applies.
...
$d\sigma/dp_\mu$
  & Final
  & Full covariance including detector variations. \\
$d\sigma/d\cos\theta_\mu$ and $d\sigma/d\theta_\mu$
  & Final
  & Full covariance; discriminates between models after the width-aware
    regularisation fix. \\
$d\sigma/d\cos\theta_\pi$
  & Final
  & Full covariance. (No $\theta_\pi$ parameterisation is extracted; only the muon angle
    is released in both forms.) \\
$d\sigma/d\theta_{\mu\pi}$
  & Final
  & Full covariance; the best-conditioned shape observable. \\
$d\sigma/dp_\pi$, two-bin
  & Final, coarse
  & Full covariance, but a two-region partition rather than a spectrum: one
...
The skim mechanism of the checklist (\S\ref{sec:request}) was exercised on two inputs that
are not beam-on: the Run-1 FHC neutrino overlay and the Run-1 beam-off sample, each
...
\begin{table}[H]
  ...
  \caption{Verification of the control-region skim (\texttt{macros/cr\_skim\_check.C},
    2026-09-06) on two non-beam-on inputs processed both in the ordinary mode and with
    \texttt{XSEC\_CR\_SKIM=1}: the Run-1 beam-off sample and the Run-1 FHC neutrino
    overlay (the ordinary processing of the latter is the sideband-instrumented file of
    \S\ref{sec:cr_fullstack}; the selection code is unchanged between the two builds).
    Counts are unweighted events. The last row is an end-to-end run of
    \texttt{univmake} on a file list in which the skim replaces one beam-on entry.}
  ...
  \begin{tabular}{lrr}
    ...
    & beam-off Run 1 & $\nu$ overlay Run 1 FHC \\
    \midrule
    input events (ordinary processing)                       & 904\,362 & 705\,700 \\
    of which signal-region                                    & 8        & 4\,124   \\
    of which in a control region and not signal-region        & 520      & 46\,858  \\
    events written by the skim                                & 520      & 46\,858  \\
    \midrule
    \texttt{XSEC\_CR\_SKIM} marker present                    & yes & yes \\
    skim events with the signal-region flag set               & 0   & 0 \\
    skim events in no control region                          & 0   & 0 \\
    run/subrun/event identifier branches in the tree          & none & none \\
    skim $=$ ordered subset $\{\lnot\mathrm{Selected}\wedge\mathrm{CR}\}$ of the full file & pass & pass \\
    \texttt{sb\_guard} accepts the skim                       & pass & pass \\
    \texttt{univmake} with the skim in its file list          & \multicolumn{2}{c}{refused (\texttt{BLINDING GUARD}, abort before any histogram is filled)} \\
    \bottomrule
  \end{tabular}
\end{table}
...
events written. The skim of the Run-1 beam-off sample is $0.06\%$ of its input, so the
skim size carries no information on the eight signal-region events removed with the
$903{,}834$ events in no control region.
...
\texttt{sb\_pi0\_final}, \texttt{sb\_nprimtrk}, \texttt{sb\_nnonproton}), the eighteen
Run-4 detector-variation samples included; no beam-on file was touched, and the frozen
...
\begin{table}[H]
  ...
  \caption{The $\pi^0$ control region without (frozen) and with the final multiplicity
    ...
    MC-statistical total of \S\ref{sec:cr_transfer}; the detector term is given with its
    MC-statistical floor from the split-sample method.}
  ...
  \begin{tabular}{lrrrr}
    ...
    target CC$+$NC~$\pi^0$              & 3331 & 2974 & 4118 & 3567 \\
    signal                              &  662 &  598 &  780 &  698 \\
    $\nu_\mu$CC~$0\pi$                  & 1011 &  953 & 1117 & 1041 \\
    other neutrino background           & 2654 & 2318 & 3183 & 2678 \\
    ...
    $T$ detector term / floor           & $15.0/12.1\%$ & $15.8/12.5\%$ & $11.6/18.0\%$ & $11.2/17.4\%$ \\
    \midrule
    shape of $E_{\rm shr}$ (frozen distribution), unit-area $\chi^2/$bins & \multicolumn{2}{c}{$0.60/10$} & \multicolumn{2}{c}{$8.3/10$} \\
    \bottomrule
  \end{tabular}
\end{table}
...
the leading-shower-energy distribution the comparison will be made on has the same shape
($\chi^2$ of $0.6$ and $8.3$ on ten bins with the two nested samples' MC statistics), and
the overlap with multi-$\pi$ is identical because multi-$\pi$ requires the cut anyway.
...
$11$--$14\%$ of the region, not a dominant population.
...
the study shows that the choice is immaterial to the region's composition, purity,
transfer uncertainty and frozen shape, so nothing about the eligibility question of
...
\begin{enumerate}\itemsep2pt
  \item \textbf{What is examined.} For each of the four regions: the event total, and the
    ...
    angle, and for the $\pi^0$ region the leading shower energy (multiplicities are not
    stored in the processed trees and cannot be added without a reprocess, which the
    frozen procedure forbids after inspection), at the binning of the released
  ...
  \item \textbf{Acceptable agreement.} The control regions pass as a set if the global
    ...
    independent flags are not themselves pass/fail criteria (with sixteen tests at these
    thresholds one false flag is expected more often than not). A single-bin excursion
  ...
  \item \textbf{Permitted responses and the rule for choosing between them.} The response
    ...
    classes $\nu_\mu$CC~$0\pi$ and CC$+$NC~$\pi^0$, ratified 2026-09-06) and only if the
    ...
    (iv)~\emph{Model revision (d).} If, with the treatment applied, the global test of
    item~3 does not pass or a flag remains, the background model is revised and renewed
    collaboration approval is required before the signal region is opened.
    The pass/fail test of item~3 is always the pre-fit one; any post-treatment goodness of
    fit is reported separately and labelled as such. FHC and RHC share one background
