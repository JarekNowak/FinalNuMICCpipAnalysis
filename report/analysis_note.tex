...
\begin{titlepage}
  ...
  \begin{abstract}
    ...
    %
    The extraction is validated by a same-sample self-closure, and cross-checked against
    the D'Agostini iterative-Bayesian method: its flat, iteration-stable integral
    ($27$--$33\%$ above Wiener-SVD, and flat to $\le2.1\%$ across observables where the
      Wiener-SVD spread is $16$--$37\%$) confirms that the
    ...
    framework on the resolution-optimised binning of Sec.~\ref{sec:binopt}. One
      ...
      the unfolded integral sits above the fake-data truth by $11\%$ on average over the
      eighteen inclusive extractions, with a per-observable scatter of $0.93$--$1.23$ and
      no configuration systematically better behaved than another (means $1.12$, $1.08$,
      $1.12$). This is a Wiener--SVD normalisation offset of the same order as the physics
      differences being measured (\S\ref{sec:limitations}).
  \end{abstract}
  \vfill
  \tableofcontents
\end{titlepage}
...
\begin{table}[H]
  ...
  \caption{Combined FHC$+$RHC fake-data closure for the eleven proton-tagged observables at
    binning and columns as in Table~\ref{tab:wtki_fhc}. All eleven close positive and
  ...
  \begin{tabular}{lccc}
    ...
    Observable & $\sigma_\mathrm{int}$ & unf./truth & $\chi^2/\mathrm{ndf}$ \\
    \midrule
    $W_{\pi p}$        & $0.529$ & $1.17$ & $1.41/6$ \\
    $W_\mathrm{had}$   & $0.554$ & $1.30$ & $6.38/6$ \\
    $\delta p_T$       & $0.646$ & $1.06$ & $0.16/2$ \\
    $\delta\alpha_T$   & $0.531$ & $1.15$ & $0.20/2$ \\
    $\delta\phi_T$     & $0.657$ & $1.08$ & $0.30/2$ \\
    $p_n$              & $0.497$ & $1.08$ & $0.24/2$ \\
    \midrule
    $p_\mu$            & $0.553$ & $1.21$ & $0.74/7$ \\
    $p_\pi$            & $0.511$ & $1.17$ & $3.17/5$ \\
    $\cos\theta_\mu$   & $0.505$ & $1.14$ & $2.26/5$ \\
    $\cos\theta_\pi$   & $0.478$ & $1.17$ & $4.30/4$ \\
    $\theta_{\mu\pi}$  & $0.481$ & $1.15$ & $0.17/5$ \\
  \bottomrule
  \end{tabular}
\end{table}
...
\paragraph{A covariance-dependent normalisation offset of order 11\%.}
On fake data the truth is known exactly, so the unfolded integral should reproduce
it. It does not. Over the eighteen inclusive extractions the ratio averages $1.106$,
with a per-observable scatter of $0.93$--$1.23$ (standard deviation $0.081$) and
configuration means of $1.12$ (FHC), $1.08$ (RHC) and $1.12$ (combined). The offset
is the same order as the physics differences this analysis sets out to measure ---
the RHC/FHC truth ratio itself is $0.785$ --- so it cannot be dismissed as a small
correction. It is \emph{not} uniform, and an earlier draft overstated the case by
quoting $1.13$ for all three configurations; that figure is correct for $p_\pi$
alone and is withdrawn.

The comparison itself is sound: as established above, the stored truth curve is
already $A_C$-smeared, so unfolded result and truth live in the same space.

\paragraph{What drives it.}
The offset tracks the \emph{covariance}, not the smearing. Three independent lines
point the same way.

First, it does not follow $A_C$. If the ratio of integrals were set by the smearing,
it would track the $A_C$ row sums; those span $0.24$--$0.97$, a factor of four, while
the ratio spans only $0.93$--$1.23$. Fitting both descriptions to the eighteen
points, a constant ratio beats ``ratio $=$ row sum'' by a factor of six in rms
residual ($0.081$ against $0.503$).

Second, it correlates with the size of the systematic covariance, and negatively:
against the flux term, which dominates it at $22$--$33\%$, the correlation is
$r=-0.643$. Larger covariance, smaller unfolded integral.

Third, and decisively, changing the covariance moves it. Removing a single term ---
the detector systematic --- from the covariance while leaving everything else alone
shifts the ratio by $2$--$12\%$ in five of six test extractions (mean $5.6\%$,
range $-5.9\%$ to $+11.7\%$). The same effect is visible in the detector discussion
above, where restricting the detVar input moves an extracted cross section by $8\%$.

The mechanism follows. The covariance enters the Wiener filter, so the strength of
the regularisation --- and with it the amount by which the filter suppresses the
result --- depends on the systematic budget. $A_C$ is constructed to describe what
the filter does to the \emph{shape}, and applying it to the predictions is what makes
the shape comparison meaningful. It does not absorb the accompanying change in
normalisation, which is why the closure does not return unity even though every curve
is correctly smeared.

\paragraph{What this means for the measurement.}
The offset is a property of regularised unfolding as configured here, not a bug and
not a failed closure. But it is not harmless: an $11\%$ normalisation shift that
varies with the systematic budget cannot be quoted as a cross section without either
being corrected for or carried as an uncertainty. Two consequences follow directly.
A configuration with a larger systematic budget will report a systematically smaller
integral, so comparisons between FHC, RHC and combined inherit part of this effect.
And because the offset moves when a systematic is added or removed, the central value
is not independent of the systematic model --- which is the opposite of what a
systematic uncertainty is normally taken to mean.

The same-sample self-closure of App.~\ref{sec:identity} recovers $0.94$--$1.06$
because it is run with a different covariance, the detector-variation POT offset
being absent by construction. That is consistent with everything above, and it is the
natural place to start in quantifying the size of the correction needed: repeating it
across the systematic budget would map the offset as a function of the covariance
directly. Supporting numbers in \texttt{report/offset\_vs\_AC.tsv}.

\paragraph{\texorpdfstring{The $A_C$ ``inconsistency'' was a misreading, now resolved.}{The A-C inconsistency was a misreading, now resolved.}}
An earlier version of this section reported an inconsistency: for the proton-tagged
$p_\mu$ in FHC the fake-data truth integrates to $0.476$, the unfolded result to
$0.553$, and $A_C$ applied to that truth to $0.376$, so that $\hat{x}=A_C
x_\mathrm{true}$ appeared to predict $0.79\times$ where $1.16\times$ was measured.

That was an error on our part, not a defect in the extraction.
\texttt{CrossSectionExtractor} applies $A_C$ \emph{in place} to every entry of its
prediction map, the fake-data truth among them, before any of them is written out.
The curve stored as the truth is therefore \emph{already} $A_C$-smeared, and
multiplying it by $A_C$ a second time --- which is what the comparison above did ---
simply double-smears it. This also explains why explicitly adding a second
\texttt{apply\_ac} to that curve moved the closure ratio further from unity
($1.09$--$1.24 \to 1.54$--$1.60$) rather than toward it.

The evidence is unambiguous across all eighteen inclusive extractions. If the stored
truth were raw, the ratio of integrals would track the $A_C$ row sum, which spans
$0.24$--$0.97$. It does not: the ratio stays between $0.93$ and $1.23$ while the row
sum varies by a factor of four, and all eighteen points sit closer to unity than to
their own row sum. $\cos\theta_\mu$ in RHC is the clearest case, with a row sum of
$0.24$ and a ratio of $0.98$.

The consequence matters for how the offset above should be read. Unfolded result and
truth curve live in the \emph{same} space, so comparing their integrals directly is
correct, and the $11\%$ excess is a genuine normalisation bias in the extraction
rather than an artefact of comparing quantities in different spaces. Supporting
numbers in \texttt{report/offset\_vs\_AC.tsv}.

\paragraph{\texorpdfstring{Why the combined detector systematic sits low.}{Why the combined detector systematic sits low.}}
After the per-mode detVar pooling fix the combined detector uncertainty tends toward
the \emph{low} end of the band spanned by its two inputs --- on average $39\%$ of the
way up that range --- and for $\cos\theta_\mu$ it falls just below both ($23.7\%$
against $26.1\%$ and $32.5\%$). At first sight a combined exposure returning less
detector uncertainty than either input looks wrong.

It is not. Running the combined extraction with only the FHC detVar set, and then
with only the RHC set, gives $27.8\%$ and $26.0\%$ for $\cos\theta_\mu$; pooling
both gives $23.7\%$. Each individual set therefore carries more uncertainty than the
two together. That is the expected behaviour if each measured detector shift is the
true shift plus a statistical fluctuation from the finite size of the detVar sample:
pooling the two samples averages the fluctuation down while leaving the true shift
alone. On that reading the pooled number is the \emph{better} estimate and the
single-mode numbers are mildly inflated, which is consistent with the combined value
sitting low in every observable rather than only in $\cos\theta_\mu$.

One consequence deserves attention. Changing only which detVar files enter the fit
moves the extracted $\cos\theta_\mu$ cross section from $0.758$ to $0.822$, an $8\%$
swing, because the detector covariance feeds the Wiener filter and therefore the
regularisation. The detector systematic is not a post-hoc error bar here; it changes
the central value.
...
The unfolded fake data sits at $1.85$--$1.90$ times the $A_C$-smeared GENIE~v3
curve, averaged over the six inclusive observables and consistent across the three
configurations. This is a statement about the \emph{injected} sample rather than
about nature: the fake data is thrown from NuWro, which predicts roughly twice the
GENIE CC$\pi^{\pm}$ rate in this phase space, and the ratio also carries the
uniform $13\%$ extraction offset of \S\ref{sec:limitations}. Against the MicroBooNE
tune the same ratio is $1.35$--$1.45$.

An earlier version of this section reported $\approx0.65$--$0.70$ times GENIE, in
the opposite direction and described as consistent with generator over-prediction.
That number cannot be reproduced from any current output and its provenance is not
established; it is withdrawn rather than restated. No claim about the sign of a
data-generator discrepancy can be made from this note in any case, because no
beam-on data is unblinded. This measurement provides:
\begin{itemize}[noitemsep]
...
\paragraph{What the combined configuration required.}
The combined measurement initially returned an integrated cross section below both
of its own inputs, which a combined exposure cannot do. The cause was the detector
systematic: a combined configuration carries one detVar set per horn current, and
each was independently scaled to the \emph{full} data exposure before the two were
summed, so the exposure was counted twice and the detector covariance came out
roughly double ($42.3\%$ of the prediction against $23.2\%$ and $27.4\%$ for its
inputs). Pooling the detVar POT brings it to $24.6\%$, inside the range spanned by
the inputs, and restores the closure: the combined truth moves from $0.910$ to
$1.027$ of the POT-weighted average of its inputs (\S\ref{sec:combtruth}). The
FHC-only and RHC-only cross sections are bit-identical before and after, so only
the combined configuration was ever affected.

\paragraph{Required for publication.}
The four open items of \S\ref{sec:limitations}, of which the uniform $13\%$
Wiener--SVD normalisation offset is the most consequential, since it is the same
order as the physics differences being measured. Beyond those: native systematic
propagation in the custom pipeline (flux, cross-section, and detector-response
multisims, currently imported from the framework for the unfolding covariance) and
inclusion of Runs~2, 4, and~5 beam-on data ($\approx 5.4\times$ additional
exposure) are needed before a real-data result can be published.  With the EXT cancellation and full covariance in place the unfolded
