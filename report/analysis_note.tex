...
\begin{itemize}\itemsep1pt
  ...
  \item[$\square$] A control-region skim of the beam-on files: the signal-region flag
    and the control-region flags are branches of the same processed tree, so processing
    a beam-on file for the control regions would also make its signal-region events
    available. Before opening, \texttt{ProcessNTuples} is to write beam-on events to a
    separate skim only if they fail the signal-region selection (the signal-region flag
    is never set in that file), the sideband macros read that skim only, and the
    extraction refuses it (no Monte Carlo truth, \texttt{XSEC\_UNBLIND} unset). The
    skim is produced by one named analyser and the full beam-on processed files are not
    written until the signal-region approval. This mechanism does not yet exist.
...
\begin{enumerate}\itemsep2pt
  ...
  \item \textbf{Permitted responses and the rule for choosing between them.} The response
    ...
    passes, the excursion is treated as a reconstruction issue, not a model correction:
    it is documented, the affected run period is excluded from \emph{both} the
    control-region comparison and the measurement with its exposure removed from the
    normalisation, and the region is re-tested; no cut or binning may be changed and a
    ...
    $p<0.01$. Its uncertainty is the transfer-factor uncertainty of
...
The algebraic identity $UR=A_C$ holds to $4.4\times10^{-6}$ (\suppref{Sec.}{sec:crosschecks}), which tests the unfolding
