...
normalisation manifest (\suppref{Sec.}{sec:pipeline}) asserts that every (beam-off file, run) pair
...
each extraction uses the complete systematic covariance, so its $p$-values are not a sensitive test
...
the central-value weighting only; the full-stack variant of \suppref{Sec.}{sec:cr_fullstack}
...
procedure of \suppref{Sec.}{sec:cr_protocol}.
\end{quote}
...
test, and where item~4 of \suppref{Sec.}{sec:cr_protocol} permits to correct the normalisation
...
cross sections are given in \suppref{Table}{tab:ppi_partial}.
