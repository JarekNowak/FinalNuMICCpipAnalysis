...
\begin{titlepage}
  ...
  \begin{abstract}
    ...
    pseudo-data thrown from a reweighted interaction model --- a model-distortion closure, not an independent-generator one --- unfolded with the nominal response biases
...
\begin{itemize}\itemsep1pt \setlength{\parskip}{0pt}
  ...
  \item \textbf{Validated:} the implementation, the size of the statistical uncertainties, and
    robustness to a reweighted interaction model (a model-distortion closure with GENIE reweighting, FHC and RHC; an independent-generator closure with NuWro is running) --- no bin biased by more than
    $0.8\sigma$, though the two edge bins move by $30$--$60\%$ (\S\ref{sec:altmodel}). Not tested for
    the proton-tagged family.
...
$d\sigma/dp_\mu$ is being re-extracted with that component at $\pm5\%$ and $\pm10\%$ (the binned
quantity only; the precomputed selection flag does not move, which is the limit of this bound).
The shift relative to the quoted uncertainty will be reported here as the bound. The
