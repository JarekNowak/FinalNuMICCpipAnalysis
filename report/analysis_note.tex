...
\paragraph{An upstream normalisation offset of order 13\%, plus regularisation scatter.}
On fake data the truth is known exactly, so the unfolded integral should reproduce
it. It does not, and running the extraction four ways separates the effect into two
pieces that behave quite differently.

\begin{center}\small
\begin{tabular}{lccc}
\toprule
Unfolding configuration & mean $\hat{x}/x_\mathrm{true}$ & spread & mean $A_C$ row sum \\
\midrule
Wiener--SVD, first derivative  & $1.073$ & $0.78$--$1.18$ & $0.61$ \\
Wiener--SVD, second derivative & $1.106$ & $0.93$--$1.23$ & $0.65$ \\
Wiener--SVD, identity          & $1.127$ & $1.08$--$1.15$ & $0.74$ \\
D'Agostini, 4 iterations       & $1.125$ & $1.10$--$1.15$ & $0.99$ \\
\bottomrule
\end{tabular}
\end{center}

\noindent \textbf{A common offset of $12$--$13\%$ survives every configuration.} It
is present in D'Agostini, which is a different algorithm with different
regularisation and an $A_C$ that is very nearly normalisation-preserving (row sum
$0.99$ against $0.65$ for the default). An effect common to two unrelated unfolding
methods cannot be a property of either, so it is \emph{upstream} of the unfolding.
The two configurations that distort least agree closely --- identity $1.127$,
D'Agostini $1.125$ --- and give the cleanest estimate of its size, $\approx\!12.6\%$.

\textbf{The observable-to-observable scatter, by contrast, is the regularisation.}
Identity and D'Agostini return ratios within $0.05$--$0.07$ of each other across all
six observables; the derivative regularisations spread them over $0.30$ and $0.40$.
That scatter is the $A_C$ smearing, and it is the reason the integrated cross section
varies between observables that cover the same phase space.

\paragraph{On the choice of regularisation.}
Moving from the second derivative to the first is the wrong direction: it nearly
doubles the spread ($0.30\to0.40$) and introduces a $0.78$ low outlier, while
shifting the mean only from $1.106$ to $1.073$. Identity regularisation gives much
the best closure uniformity, at the cost of the smoothing the derivative operators
provide, and its agreement with D'Agostini is a useful cross-check on both. The
second derivative is retained as the default; the case for revisiting that is the
$0.30$ spread, not the offset, which identity does not remove either.

\paragraph{What this means for the measurement.}
The $\approx\!13\%$ excess is not an unfolding artefact and will not be removed by
choosing a different unfolder or regulariser. It is a normalisation mismatch between
the injected fake data and the truth curve it is compared against, and it must be
found before an integrated cross section is quoted --- a $13\%$ normalisation error
is larger than most of the systematic budget. The natural places to look are the
steps between event counts and a cross section that the two paths do not share:
the exposure used to scale the fake data against the one used to build the
prediction, the background subtraction, and the flux and target-number factors.

An earlier version of this section attributed the offset to the covariance entering
the Wiener filter, on the evidence that removing the detector term shifts the ratio
by $2$--$12\%$. That evidence stands, but the interpretation was wrong: the
covariance modulates the offset, it does not cause it, since the offset survives in
D'Agostini and in identity regularisation. Supporting numbers in
\texttt{report/regularisation\_scan.tsv} and \texttt{report/offset\_vs\_AC.tsv}.
