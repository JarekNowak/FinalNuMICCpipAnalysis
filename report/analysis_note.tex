...
Proton angular observables $\theta_p$, $\theta_{\pi p}$
  & Proposed secondary scope & Binning, generator predictions and all three
...
\begin{table}[H]
  ...
  \caption{RHC fake-data closure for the twelve proton-tagged observables,
    ...
    the convention-agnostic MC POT normalisation described above. The third block ($^{\dagger}$) is the two angular observables proposed as secondary scope (\S\ref{sec:wtki_ang}).}
  \label{tab:wtki_rhc}
  \begin{tabular}{lccc}
\toprule
Observable & $\sigma_\mathrm{int}$ & unf./truth & $\chi^2/\mathrm{ndf}$ \\
...
\begin{table}[H]
  ...
  \caption{Combined FHC$+$RHC fake-data closure for the twelve proton-tagged observables at
    ...
    not required to lie between them; see the discussion below. The third block ($^{\dagger}$) is the two angular observables proposed as secondary scope (\S\ref{sec:wtki_ang}).}
  \label{tab:wtki_comb}
  \begin{tabular}{lccc}
\toprule
Observable & $\sigma_\mathrm{int}$ & unf./truth & $\chi^2/\mathrm{ndf}$ \\
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{dsigma_ccpi1p_noW_RHCFULL}
...
\begin{figure}[H]
  ...
  \caption{The same two measurements as maps: $\mathrm{d}^2\sigma/\mathrm{d}X\,\mathrm{d}Y$ per cell
    ...
    equal-size and labelled by their ranges; the palette is the \texttt{kBird} scale used for the
...
claims on simulation, where the right answer is known: whether the extraction returns the truth it
was given, whether the quoted uncertainties are the right size, and whether the background fit that
the control regions feed is unbiased. The tests that do use beam-on data --- the control-region
comparison itself --- are in the control-sample note.

\subsection{Self-closure of the extraction}
