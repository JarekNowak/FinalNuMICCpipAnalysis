...
single-shower events that already resemble the signal region. That expectation was
tested on 2026-09-06 with a reprocess of the Monte Carlo alone
...
distributions themselves and nowhere else. The frozen definition, the one in the table,
is kept as the larger and more inclusive sample.
...
\midrule
\multicolumn{3}{l}{\emph{Secondary scope --- complete covariance, secondary results}}\\
\midrule
...
Proton-tagged differential results, including transverse kinematic imbalance
  ...
  & Detector systematics propagated; beam-frame reconstruction corrected;
    central values postdate the weighting fix; $A_C$ and covariance released. \\
\midrule
...
flagged on the axis, so the plotted area does not represent cross section. The
less ambiguous statement of the same result is the pair of bin-integrated partial cross
sections, $\sigma_i=(d\sigma/dp_\pi)_i\,\Delta p_{\pi,i}$, which do not depend on the
...
band is sized.  A test of the latter needs either a covariance restricted to the terms
that differ between measurement and reference, or an ensemble of throws.  Neither is done
here, and the $p$-values should be read with that limitation in mind.
...
\begin{enumerate}\itemsep2pt
  ...
  \item \textbf{Acceptable agreement.} The control regions pass as a set if the global
    $\chi^2$ of item~2(c) has $p>0.05$; the expected false-failure rate is then $5\%$ by
    construction. The per-region pulls and shape tests of items~2(a) and 2(b) are
