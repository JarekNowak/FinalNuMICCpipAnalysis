...
beamline geometry weights. This supersedes the $110$~mrad quoted earlier in this
section.} That angle says nothing about how the TPC is oriented.
...
\begin{table}[H]
  ...
  \caption{Total flux-averaged cross section from the cut-and-count method
    ...
    \emph{Generator values corrected 2026-08-17}: they previously came from truncated
    binnings ($p_\mu$ closed at $3$~GeV/$c$, $p_\pi$ at $1$~GeV/$c$) that discarded the
    tails, making every prediction $4$--$8\%$ low; see \S\ref{sec:ppi_bias}.
...
\begin{table}[H]
  ...
  \begin{tabular}{L{1.8cm} p{11cm}}
    ...
    $p_\pi$ (GeV/$c$) &
      ...
      bin open above). \emph{Changed 2026-08-17}, see \S\ref{sec:ppi_bias}. The previous
      five-bin scheme (0.175, 0.250, 0.320, 0.420, 0.550, open) had migration diagonals of
      $96/35/21/10/6\%$, so only its first bin met the $>68\%$ usable-bin criterion; the
      two-bin scheme gives $91/76\%$, clearing it in both. (An earlier draft of this table listed
      0.175, 0.20, 0.30, 0.40, 0.50, 1.00, which never matched the configuration files.)
      \\[4pt]
    $\cos\theta_\mu$ &
