...
\begin{titlepage}
  ...
  \begin{abstract}
    ...
    framework on the resolution-optimised binning of \suppref{Sec.}{sec:binopt}.
      ...
      integrated cross section rather than a resonance shape; its covariance is otherwise complete with respect to the
...
\begin{table}[H]
  \centering
  \caption{Control-region definitions in terms of the selection stages. The
    common preselection \textit{pre} is software trigger, reconstructed vertex in the
    fiducial volume, topological score $>0.67$ and a qualifying muon candidate
    (Cuts~1--4 of \S\ref{sec:selection}); \textit{shower} is the shower veto (Cut~8),
    \textit{final} the non-proton multiplicity cut (Cut~10), \textit{contained/gap} the
    pion containment and three-plane requirements (Cuts~5--7), \textit{opening} the
    $\theta_{\mu\pi}<2.6$~rad cut (Cut~9), and $n_\pi$ the number of qualifying contained
    pion candidates. The signal region is \textit{pre}, \textit{contained}, \textit{gap}, \textit{shower},
    \textit{opening} and \textit{final} with $n_\pi=1$. ``Dropped'' lists nominal requirements not
    applied in the region; ``target'' the signal-region background the region is meant to
    constrain. Each region is stored as a flag alongside the signal-region flag when the
  ...
  \begin{tabular}{p{1.5cm}p{5.2cm}p{3.0cm}p{4.6cm}}
    ...
    \midrule
    CC$0\pi$ & \textit{pre}, \textit{shower}, \textit{final}; $n_\pi=0$ & contained, gap, opening (no pion to define them) & CC$0\pi$ events with a proton or other track taken as the pion; pion absorption feed-in \\
    multi-$\pi$ & \textit{pre}, \textit{final}; $n_\pi\ge2$ & shower, contained/gap of the extra pion, opening & multi-pion events with one pion lost \\
    $\pi^0$ & \textit{pre}, shower veto failed; $n_{\rm shower}\ge1$ & contained, gap, opening, final & CC/NC~$\pi^0$ with both photons missed; shower-veto efficiency \\
    cosmic & \textit{pre}, \textit{contained}, \textit{gap}, \textit{shower}, \textit{final}; opening-angle cut inverted & opening only & residual beam-off (cosmic) in the signal region; its normalisation \\
    \bottomrule
  \end{tabular}
\end{table}
...
then tested on the Monte Carlo alone
...
detector-variation samples (\S\ref{sec:cr_transfer}). It should be noted that both
...
software trigger (46\,810/46\,810 events confirmed numerically).
...
(the true-bin definition places no upper edge on it); the $1\GeVc$ that
...
(with the weight rule of Eq.~\eqref{eq:safeweight}),
...
numuMC and detVar samples therefore carry a per-mode stored exposure
(FHC $2.158\times10^{22}$, RHC $2.782\times10^{22}$, $=S_\mathrm{mode}\times
D_\mathrm{comb}/D_\mathrm{mode}$) so the framework's data-over-stored-exposure scaling
reduces to $D_\mathrm{FHC}/S_\mathrm{FHC}$ and $D_\mathrm{RHC}/S_\mathrm{RHC}$;
...
files: the number of events carrying both the signal-region
...
exercise the exact path that beam-on data will take, the same comparison was run with
pseudo-data consisting of the neutrino-MC throw \emph{plus} an independent Poisson throw of
the beam-off and dirt expectations, compared with the complete prediction. The data/prediction ratios are $0.996$, $1.047$,
...
beam modes, the frozen distributions exactly as they will be drawn at unblinding, with the same full-stack pseudo-data in place of the
beam-on points.
...
\begin{figure}[H]
  ...
  \caption{FHC control-region distributions at the frozen binning
    (the frozen plotting list): by row, CC$0\pi$, multi-$\pi$, $\pi^0$ and
...
region by background class and the transfer factors.
...
sideband-instrumented files with the same weight conventions as the extraction and the detector-variation term from the Run-4
...
\begin{table}[H]
  ...
  \caption{Normalisation uncertainty (\%) of the neutrino-MC prediction in the signal region
    ...
    extraction's weight conventions, and the detector term from the
    Run-4 detector-variation samples, which carry the region flags (eight variations against the detector-variation central value, summed in quadrature as
    ...
    the yields agree between the tables.}
  \label{tab:cr_syst}
  \small
...
\begin{table}[H]
  ...
  \caption{Transfer factor $T=N^{\mathrm{SR}}/N^{\mathrm{CR}}$ of each control region's
    ...
    every universe so that the numerator--denominator covariance is included. The flux term, $15$--$20\%$ on
    ...
    the detector entries, estimated from two disjoint halves of the variation samples, is given in the text; for the adopted CC$+$NC~$\pi^0$
    class it is quoted in parentheses after the detector entry and, where it exceeds the
    entry, it replaces it in the total. }
...
separate set of processed files that carry the alternative region flag and the two
multiplicities, the Run-4
...
Table~\ref{tab:sideband_yields} and the uncertainties of \S\ref{sec:cr_transfer}. Table~\ref{tab:cr_pi0_final}
gives the two definitions side by side.

\begin{table}[H]
  \centering
  \caption{The $\pi^0$ control region without (frozen) and with the final multiplicity
    cut, full exposure, CV-weighted and per-run scaled. Purity and contamination are
...
\begin{enumerate}\itemsep2pt
  \item \textbf{What is examined.} For each of the four regions: the event total, and the
    ...
    non-proton multiplicities are stored by the frozen skim,
    ...
    the frozen plotting list and shown, with full-stack pseudo-data, in
  ...
  \item \textbf{Statistics.} Three quantities are computed for each region and beam
    ...
    the protocol implementation (flux, cross-section and re-interaction multisims
    ...
    the sixteen pulls lie within $\pm0.5$ and all twenty-two shape tests have $p>0.47$. The same run gives the power of the
  ...
  \item \textbf{Acceptable agreement.} The control regions pass as a set if the global $\chi^2$ of item~2(c) has $p>0.05$; the
    ...
    (Fig.~\ref{fig:cr_null_calibration}).
...
The proton-tagged selection inherits its cut logic from the inclusive selection, and with it
the four sideband definitions of \S\ref{sec:sidebands}: the reprocessed proton-tagged
files already carry the four region flags. No new definitions or reprocessing are
...
flag in the proton-tagged selection and a reprocess. Slicing a measured W/TKI observable is
...
implemented as a dedicated selection that inherits the full
...
direction cosines $\hat n$, the muon-hypothesis range and multiple-scattering momenta and
the proton-hypothesis calorimetric kinetic energy.

\emph{Candidate three-momenta.} The muon momentum magnitude is range-based when the muon
track ends inside the containment volume (\S\ref{sec:fv_where}), and
multiple-Coulomb-scattering based otherwise.
The charged pion, being minimum-ionising, starts from the muon-hypothesis range momentum, converted to the pion hypothesis at fixed kinetic
...
$p_p=\sqrt{T_p^2+2m_pT_p}$ with $T_p$ the calorimetric kinetic energy. Each
...
MINERvA prescription~\cite{minerva_tki} as implemented in the framework's transverse-kinematics tool (option~1):
\[
...
\captionof{table}{FHC (Runs 1, 2, 4, 5): bin-averaged fractional uncertainty (\%) by
  ...
  (2026-09-06 release). ``Prediction total'' excludes the data
...
Every released number is produced by the \textsc{XSecAnalyzer} chain in four stages that
are re-runnable from the raw PeLEE ntuples:
\begin{enumerate}\itemsep1pt
  \item the event processing applies the selection(s) to each PeLEE ntuple and writes the
    processed event files, for the neutrino overlays, the dirt overlay and the per-run
    beam-off samples;
  \item the universe builder constructs, per observable and configuration, the true and
    reconstructed histograms of every systematic universe from the files listed in that
    configuration;
  \item the unfolder sums the universes with the per-run exposure scaling, builds the
    covariance, unfolds, and writes the closure sidecar, the systematics dump and the
    matrix tables;
  \item the export step writes the data release (curves, $A_C$ matrices, covariance
    decompositions); the figure macros draw from the closure sidecars; the one-bin total
    uses the same chain with the single-bin configurations; the count-level tables (cut
    flow, cut-and-count check, control-region yields) are produced with the same weights
    and scalings.
\end{enumerate}
...
\begin{itemize}\itemsep1pt
  \item A normalisation manifest reads the authoritative configuration files and asserts
  ...
  \item A coupling assertion in the unfolder refuses a configuration whose declared
    detector variations are not present in its file list.
  \item A staleness check refuses to ship any released artefact or result figure older
    than the extraction it depicts, and compares the integrals printed on the two-bin
    figures with their sidecars.
  \item A table check parses every result table in this note and fails if a value
    disagrees with the released results table, if a table does not sit under its own
    label, or if a row's column count differs from its header.
  \item The blinding guard: a beam-on file without MC truth is real data and is refused
    unless an explicit unblinding switch is set; the same detection selects the beam-off
    treatment (added to pseudo-data, subtracted from real data), so no manual switch
    exists to forget.
\end{itemize}
...
edge; the $1$~GeV/$c$ listed above is the
