...
{\footnotesize\ttfamily release 2026-09-06 \textperiodcentered\ 51 differential extractions $+$ 6 one-bin totals \textperiodcentered\ blind\par}
  ...
  \begin{abstract}
    ...
    framework on the resolution-optimised binning of \suppref{Sec.}{sec:binopt}.
      ...
      but unpropagated terms are listed in \S\ref{sec:limitations}).
      ...
      best-conditioned one.  Separately, the release was built from the processed ntuples with every cached
      universe deleted, and an independent rebuild of the same kind reproduced it
      bit-identically, so the released numbers are regenerable from the raw inputs rather
      than from a preserved intermediate.  The ensemble
      ...
      %
      \textbf{Companion documents.}  This note describes the analysis as it stands and the
      reasons for its choices. The studies behind those choices --- the pion-momentum
      estimator survey, the binning and threshold optimisation, and the closure programme
      --- are in the technical supplement. The history of the analysis, every correction
      that changed a result with the values before and after, is in the change log and is
      not repeated here.  The additional-smearing and
...
cross section $\sim\!5$--$6\%$ low. The integrated values were validated four independent
ways: standalone NuWro and GENIE event generation on the same flux, the flux author's
numbers, and a GENIE closure, documented in \texttt{generator\_predictions/}.
...
\end{equation}
The gate counts are the analyser's trigger table, released in
\texttt{data\_release/ext\_gates.tsv}: Run~1 $4\,582\,248.27$,
...
(\texttt{swtrig~==~1}); the CRT-era beam-off files (Runs~4--5) carry a vestigial
\texttt{swtrig} branch that is zero throughout, so for files that also carry
\texttt{swtrig\_pre} that branch is bound instead. MC events with \texttt{swtrig~==~0} are
...
pipeline, kept as an independent cross-check of the selection, implements the
byte-identical cut sequence. The selection comprises ten sequential
...
The proton-hypothesis range \emph{kinetic energy} (\texttt{trk\_energy\_proton\_v}) is
not used: it is a different physical quantity from the momentum on the true axis, so
...
Table~\ref{tab:cutflow} above and are quoted from there.
\begin{itemize}[noitemsep]
...
(per-run gate ratios, \S\ref{sec:mc}). A single per-mode factor would differ from the
...
(Table~\ref{tab:sideband_yields}), in FHC $6.0$ signal, $69$ neutrino background, $53$
...
\begin{table}[H]
  ...
  \caption{Selection-optimisation study across the phase-space iterations, on the Run-1
    ...
    Table~\ref{tab:cutflow}. The beam-off column uses the pooled beam-off scaling of that
...
width correction matters: on these binnings, whose widths vary by up to a factor of
seven, a uniform-grid stencil penalises the density's curvature by the wrong weights
...
standalone replica (\texttt{xsec/WienerSVD.h}, \texttt{RunWienerSVD\_FW}) exists for
cross-pipeline checks; it imports the framework's
...
correction twice). The implementation
...
for the six observables (the $p_\pi$ panel at the rejected five-bin binning, as a
...
\begin{figure}[H]
  ...
  \caption{Smearceptance matrices $S_{ij}=\varepsilon_j M_{ij}$, FHC, exactly as passed to
    ...
    binning; the $p_\pi$ panel is deliberately the \emph{rejected five-bin} response, kept as the diagnostic that shows reconstructed events piling into the lowest bin from
...
(\S\ref{sec:pipeline}). The point matters because a second application would over-smooth
...
This subsection states plainly what each result is and on what basis, so that no number
in this note has to be interpreted by working out its provenance. All fifty-one
extractions carry the complete configured systematic covariance, detector variations
included (``complete'' relative to the configured uncertainty model; the identified
terms that are not in it are listed in \S\ref{sec:limitations}). The proton-tagged
...
$d\sigma/d\cos\theta_\mu$ and $d\sigma/d\theta_\mu$
  & Final
  & Complete configured covariance; discriminates between models. \\
...
Proton-tagged integrated cross section
  ...
  & Detector systematics propagated, closure $\chi^2$ recomputed, $A_C$ and covariance
    released. \\
Proton-tagged differential results, including transverse kinematic imbalance
  ...
  & Detector systematics propagated; angles and transverse quantities in the beam frame;
    $A_C$ and covariance released; the
...
$0.02$--$0.2$ to $0.5$--$0.8$, much nearer the expected value (a study on one
release; the released closure values are those of Tables~\ref{tab:fhc}--\ref{tab:comb},
and it is the effect rather than the values that matters here):
...
(\texttt{throw\_ensemble\_fhc.C}, with the weight rule of Eq.~\eqref{eq:safeweight}),
...
\begin{center}\small
\captionof{table}{FHC (Runs 1, 2, 4, 5; data POT $8.857\times10^{20}$): integrated cross section, the two largest systematic terms, the total, and the fake-data closure against $A_C$-smeared truth. All values are read from the released covariance and results table; the anatomy of the flux term is in \S\ref{sec:fluxanatomy}. \releasestamp}
    \label{tab:fhc}
...
fourteen numuMC files carry the \emph{same} $600$ PPFX multisim universes, which the universe maker accumulates index-matched across both horn
...
correct multisim treatment, superior to any hand-built block covariance. The detector
...
\captionof{table}{Integrated (unfolded) cross section $\sigma_\mathrm{int}$ over the
  ...
  establish it directly). All entries come from one pass of the current code with the
cached universes rebuilt. The MC normalisation is evaluated run by run using the sum of the
  ...
  handles both conventions without weighting a run by its split multiplicity; FHC (one
  MC file per run) is unaffected by the convention. The final row gives the alternative
...
The check is sensitive: a curve that carried $A_C$ twice would miss this expression by
up to $55\%$, which is why it is part of the release procedure.
...
\begin{table}[H]
  ...
  \caption{Normalisation uncertainty (\%) of the neutrino-MC prediction in the signal region
    ...
    the yields agree between the tables. \releasestamp}
  \label{tab:cr_syst}
  \small
...
\begin{table}[H]
  ...
  \caption{Verification of the control-region skim (\texttt{macros/cr\_skim\_check.C},
    ...
    beam-off sample (whose software-trigger branch is bound differently,
    \S\ref{sec:mc}) and the Run-1 FHC neutrino overlay (the ordinary processing
...
comparison with the definition that retains it was produced from a reprocess of the Monte Carlo, the dirt and the per-run beam-off samples into a
...
(\S\ref{sec:cr_1p_scope}). See Sec.~\ref{sec:status_matrix}.
\end{quote}
...
score, required to be below $0.05$) among the track-like
...
One normalisation point is specific to this subsample and worth stating. The processed
ntuples use two POT conventions: the files behind the inclusive selection store the
\emph{total} run MC POT duplicated in every file of a multi-file run, whereas the
proton-tagged files store each file's \emph{own} POT. Normalising by the individual file
POT would be correct for the first set but would scale every split of a multi-file run to
the full run exposure in the second, inflating the simulated background by the split
multiplicity ($\sim\!3\times$ for RHC) and driving the background-subtracted spectrum
negative; normalising by the summed per-file POT has the mirror problem. The analysis
therefore normalises each run by the sum of the \emph{distinct} per-file POT values
within that run, which reduces to the correct run total under either convention; FHC,
with one MC file per run, is unaffected by the choice.
...
quadrature as two-sided unisim variations. Evaluated in a dedicated pass
(\texttt{ccpi\_systcalc\_numi\_beamline.conf}) it is $1.6$--$3.3\%$ across the observables
and sub-dominant; it is \emph{not} part of the released covariance, whose configuration
(\texttt{ccpi\_systcalc\_numi.conf}) omits the beamline block. Added in quadrature to the
...
\begin{enumerate}\itemsep1pt
  ...
  \item \texttt{univmake} builds, per observable and configuration, the true/reco
    ...
    \texttt{file\_properties} (\texttt{slurm/slurm\_extfix.sbatch} for a full rebuild on the cluster, one array job for
    all 51).
...
Five automated guards protect the normalisation, the currency of released artefacts,
the result tables and the blinding (the change log records the incident that motivated
each):
\begin{itemize}\itemsep1pt
  ...
  \item \texttt{report/check\_tables.py} parses every result table in this note and fails
    ...
    its own label, or if a row's column count differs from its header.
...
\end{itemize}
The release was built from the processed ntuples with every cached universe deleted, in
a single pass, so no preserved intermediate exists; a clean-cache rebuild of the same
kind reproduced an earlier release bit by bit, and repeating that comparison on the
release to be unblinded is on the pre-unblinding list (\S\ref{sec:limitations}).
...
release to be unblinded is a prerequisite (\S\ref{sec:pipeline}). This is a different
...
\S\ref{sec:pipeline} exist because of them. This note describes the analysis as it
