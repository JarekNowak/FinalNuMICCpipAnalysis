...
\newcommand{\releasestamp}{[current frozen release]}
...
This section is the self-contained case for the one decision requested of the
collaboration at this stage: permission to inspect beam-on data in four
reconstruction-level control regions of the inclusive $\mathrm{CC}\,1\mu1\pi Xp$
selection while the signal region stays blind. Everything here is expanded in
\S\ref{sec:sidebands}; the numbers are those of the frozen release identified in the box
below.

\medskip\noindent\fbox{\parbox{0.97\linewidth}{\textbf{Analysis state.} Numerical release
2026-09-06 (document revision 2026-09-07); repository tag \texttt{cr-freeze-2026-09-06};
fake data: one Poisson throw per run of the central-value prediction with the unified
weight rule (\S\ref{sec:mc}); binning: the released edges of Appendix~\ref{app:binning}
(five-bin forward-split $\cos\theta_\pi$, two-bin $p_\pi$); normalisation: per-run
...
manifest \texttt{MANIFEST.sha256} (\texttt{b8ebc69e3bc91434}). Every table marked
``current frozen release'' is generated from this state.}}
\medskip
...
brevity: it is cosmic-\emph{enriched}, $34$--$41\%$ beam-off, not cosmic-pure).
...
are dropped as listed in Table~\ref{tab:cr_defs}. Each region is disjoint from the
signal region at reconstruction level (demonstrated event by event,
\S\ref{sec:cr_overlap}); their expected signal contaminations are $8$--$24\%$. The
...
discrepancy are frozen as written in \S\ref{sec:cr_protocol}. Control-region data will be
...
the signal region is opened. The proton-tagged control regions are \emph{not} part of
this request (\S\ref{sec:cr_1p_scope}).

\paragraph{What each region can and cannot do.} The physics roles, which the
transfer factors of \S\ref{sec:cr_transfer} decide, are these.
\begin{itemize}\itemsep2pt
  \item \textbf{CC$0\pi$} is the high-statistics validation of the dominant neutrino
    background class: CC$0\pi$ events with a proton or other track taken as the pion are
    $244$ of the $637$ FHC neutrino-background events ($38\%$). The region has $72\%$
    target purity ($65\%$ of the full stack) and $0.6\%$ raw statistical precision. It
    validates the normalisation and kinematics of the underlying CC$0\pi$ population,
    \emph{not} the proton-to-pion misidentification probability that transfers that
    population into the signal region: the transfer carries a $31$--$34\%$ uncertainty
    dominated by the detector variations. It is therefore a validation test rather than
    a per-cent-level constraint, and it is conditionally eligible for a fitted
    normalisation correction under \S\ref{sec:cr_protocol}. The direct test of the
    misidentification probability is an inverted pion-PID region, which does not exist
    yet and is listed among the prerequisites for opening the signal region.
  \item \textbf{$\pi^0$} tests the shower veto and the CC$+$NC~$\pi^0$ background. Its
    target purity is $50.1\%$ (FHC) and $51.0\%$ (RHC) of the neutrino MC, its transfer
    factor $0.02$ with a $17$--$19\%$ uncertainty set by the Monte-Carlo-statistical
    floor of the detector-variation samples. \textbf{For this opening the region is
    diagnostic only.} Approval to inspect it does not authorise a fitted correction; a
    proposal to fit the CC$+$NC~$\pi^0$ normalisation would require a separate
    documented review after the pre-fit result has been recorded, with the fit model,
    the leverage of the fixed components, the transfer uncertainty and its floor, and
    the expected effect on the signal-region prediction.
  \item \textbf{Multi-$\pi$} is diagnostic only. Its transfer factor is $\simeq2.5$
    (the signal region holds more of the target class than the region does) and its
    target purity $47\%$; it is not used to fit or constrain the signal-region
    normalisation, and exposes gross modelling failures and shape discrepancies.
  \item \textbf{Cosmic} validates the beam-off normalisation and shape to $\sim\!50\%$
    once the prediction uncertainties of its neutrino component are included ($17\%$
    from data statistics alone). That is sufficient because beam-off is $1.9\%$ of the
    selected signal-region prediction: a $50\%$ uncertainty on it is an effect of order
    $1\%$ on the selected sample. It is not eligible for a fitted correction.
\end{itemize}
About $29\%$ of the FHC neutrino background has no region of its own: NC and
$\nu_e$CC without a $\pi^0$, out-of-fiducial-volume events and CC$1\pi^\pm$ outside the
measured phase space (\S\ref{sec:cr_transfer}). These components keep their model
uncertainties unchanged; the CC$0\pi$ and $\pi^0$ distributions carry indirect
sensitivity to the first two through the shared reconstruction, and each is small
enough individually that no dedicated region is justified at this exposure.

\paragraph{What the test detects, and its blind direction.} The pass/fail statistic is
a global normalisation $\chi^2$ over the eight (region, mode) totals with the full
pre-fit covariance (\S\ref{sec:cr_protocol}). It detects a discrepancy confined to one
region beyond $12\%$ of the CC$0\pi$ or $\pi^0$ total and beyond $50$--$62\%$ of the
multi-$\pi$ or cosmic total, an FHC/RHC inconsistency, and, through the
normalisation-marginalised shape tests, a shape difference; a \emph{common}
normalisation shift of all regions fails it only beyond $58\%$, because the correlated
flux, cross-section and detector terms absorb it. That blind direction is acceptable
because it is the direction the signal-region measurement already carries as its flux
and cross-section uncertainty: the control regions test the background model region by
region, not the overall normalisation.
...
are for. The control-region comparison is made at reconstruction level, so it does not
depend on the unfolding or on the alternative-generator closure that is required before
the signal region is opened (\S\ref{sec:limitations}).
...
final multiplicity cut. The cut was dropped when the region was defined to avoid a
plausible topology-dependent sculpting (photon showers reconstructed as additional
track-like primaries). The dedicated comparison of \S\ref{sec:cr_pi0_final} subsequently
found no material change in target purity, transfer uncertainty or the frozen
leading-shower-energy shape. The no-cut definition is retained because it provides
$11$--$14\%$ more events, not because the study demonstrated a material bias from the
cut.

\paragraph{Points the collaboration is asked to ratify.}
(i)~The $\pi^0$ definition without the final multiplicity cut (Table~\ref{tab:cr_defs});
the comparison with the version retaining it is in \S\ref{sec:cr_pi0_final} and will
\emph{not} be used to modify the region after beam-on inspection. (ii)~The $\pi^0$
target class CC$+$NC~$\pi^0$, adopted by the analysers on 2026-09-06 (the NC$\pi^0$
...
target class's share of the \emph{neutrino-MC} prediction in the region, beam-off and
...
frozen procedure of \S\ref{sec:cr_protocol}.
...
\midrule
Control-region definitions frozen & Table~\ref{tab:cr_defs} & \\
Signal-region overlap zero, event by event & \S\ref{sec:cr_overlap} & \\
Plot list and binning frozen & \S\ref{sec:cr_protocol} item~1 & \\
Pre-fit covariance, global statistic and thresholds frozen & \S\ref{sec:cr_protocol} items~2--3; null calibration Fig.~\ref{fig:cr_null_calibration} & \\
Response to a discrepancy frozen & \S\ref{sec:cr_protocol} item~4 & \\
$\pi^0$ definition and target class ratified; diagnostic-only status recorded & this section, (i)--(ii) & conveners \\
Flux and detector-systematics contacts have reviewed their parts & pending & contacts \\
Repository tag and release checksum recorded & analysis-state box above & \\
\midrule
\multicolumn{3}{l}{\emph{Required later, before the signal region is opened}}\\
\midrule
Control-region results reviewed; any discrepancy treated under \S\ref{sec:cr_protocol} & pending & conveners \\
Alternative-generator (NuWro or GiBUU) fake-data closure on the released chain & pending, \S\ref{sec:limitations} & \\
Flux constants confirmed as PPFX-corrected central values & pending, \S\ref{sec:flux_status} & flux contact \\
MCS muon-momentum uncertainty bounded or propagated & pending, \S\ref{sec:limitations} & \\
Final empty-cache reproduction of the release to be unblinded & pending, \S\ref{sec:pipeline} & \\
Inverted pion-PID region for the CC$0\pi$ misidentification transfer & pending & \\
Inverted-proton-PID region before any proton-tagged result & pending, \S\ref{sec:cr_1p_scope} & \\
\bottomrule
\end{tabular}
\end{center}
...
and $6\%$ (multi-$\pi$) for the combined exposure. The neutrino-MC prediction in each
region carries the flux, cross-section and re-interaction uncertainties of
Table~\ref{tab:cr_syst}, including the detector variations. The regions carry a
...
appear as a $2.8\%$ excess), which is inside the $9$--$24\%$ detector-variation
uncertainty on the region prediction and so invisible to every pull of
\S\ref{sec:cr_protocol}; that is what keeps the regions blind to the signal, and the
response tree holds the signal at its central value in any case.

\subsection{Outcome: the regions were opened on 2026-09-09}
\label{sec:cr_outcome}

The request above was granted and the four regions were inspected on beam-on data. The full account
is the control-sample note; what follows is what a reader of this note needs, and it includes two
defects in the first comparison that were found afterwards and that changed its conclusions. The
sign-off checklist above was not revised when the regions were opened --- several of its
``required before the control regions are opened'' items still read \emph{pending} --- and
reconciling it with what was actually signed off is an open item.

\begin{itemize}\itemsep3pt
  \item \textbf{Two defects, both now corrected.} The sideband Monte Carlo used for the first
    comparison predated the beam-frame correction of \S\ref{sec:beamdir}: it stored
    $\cos\theta_\mu$ about the detector $z$ axis while the beam-on skim stored the beam-frame angle.
    The ``muon-angle shape failure'' reported in both CC$0\pi$ and $\pi^0$ was that artefact and is
    withdrawn; with the current sideband sample the ratio is flat. Separately, the sideband detector
    variations were compared without POT normalisation --- the knob samples do not carry the central
    sample's exposure --- which inflated the detector term by roughly an order of magnitude
    ($\sim\!20\%$ on control-region totals against $\sim\!2\%$ normalised). Transfer-factor ratios
    and the released extraction are unaffected, both normalising by exposure already.
  \item \textbf{One run is discrepant, and it is an exposure error, not a background.} With the
    corrected inputs the frozen global test fails when FHC Run~1 is included and passes without it.
    A free exposure factor on FHC Run~1 fits $0.520\pm0.020$ with $\chi^2=16/17$, consistent with
    the $0.51$--$0.62$ implied by the yield ratios themselves. This is evidence of a bookkeeping
    error in that run's exposure, not of a mismodelled background; it is an open item, and no result
    in this note depends on FHC Run~1 alone.
  \item \textbf{No background correction is warranted.} A joint FHC$+$RHC fit of the background
    class normalisations (\crref{Sec.}{sec:jointfit}) returns the CC$0\pi$ class at $0.984\pm0.191$ with
    FHC Run~1 excluded --- unity within a fifth of its own uncertainty. The measurement therefore
    keeps its nominal background prediction, and item~4 of \S\ref{sec:cr_protocol} is not invoked.
  \item \textbf{The residual deficit is not signal contamination.} The control regions sit
    $8$--$16\%$ below prediction. Adding one cut to each region --- ``far'' sidebands, which can only
    remove signal --- drives the signal fraction of the CC$0\pi$ region from $8.7\%$ to $2.1\%$ while
    keeping $58\%$ of its events, and the data-to-prediction ratio does not move ($0.84\to0.85$;
    $\pi^0$ $0.91\to0.92$). Whatever produces the deficit does not scale with signal contamination
    (\crref{Sec.}{sec:farsb}).
\end{itemize}
...
it into the selection (\S\ref{sec:request}).
...
(\S\ref{sec:cr_1p_scope}). Its clearest benefit is cosmic rejection: requiring a
...
(\S\ref{sec:sidebands_1p}). The full-exposure cut-flow, stage by stage and per
...
\begin{figure}[H]
  ...
  \begin{subfigure}[b]{0.32\textwidth}\includegraphics[width=\textwidth]{final_mupid_rhc}\caption{Muon LLR PID, RHC.}\end{subfigure}\hfill\\[6pt]
...
\begin{figure}[H]
  ...
  \begin{subfigure}[b]{0.32\textwidth}\includegraphics[width=\textwidth]{final_pipid_rhc}\caption{Pion LLR PID, RHC.}\end{subfigure}\hfill\\[6pt]
...
Total flux-averaged cross section (one-bin extraction), all three configurations
  & Analysis definition final; result blinded
...
$d\sigma/dp_\pi$, two-bin
  & Analysis definition final, coarse; result blinded
...
Proton-tagged differential results, including transverse kinematic imbalance
  & Validation products, not proposed for approval in this review & Detector systematics propagated; angles and transverse quantities in the beam frame;
...
Two-dimensional cross sections
  & Proposed secondary scope, combined configuration only & $\theta_p\times\delta p_T$ and
    $\cos\theta_\pi\times\cos\theta_\mu$; eight pairs screened and extracted, per-mode results are
    too poorly conditioned to invert (\S\ref{sec:2d}). \\
Proton angular observables $\theta_p$, $\theta_{\pi p}$
  & Proposed secondary scope, added 2026-09-18 & Binning, generator predictions and all three
    extractions complete on fake data (\S\ref{sec:wtki_ang}); $\theta_{\pi p}$ needed a gated
    reprocess of the proton-tagged tree; $\theta_p$ is rank-deficient in RHC and is proposed for
    FHC and the combined configuration only. \\
\midrule
...
physical (not unit) cross-horn correlation automatically; universe indices are shared across FHC and RHC and summed index by index, which preserves
the implemented cross-horn correlations without a hand-built block covariance. The detector
...
The regions were opened on 2026-09-09 and the outcome is summarised in \S\ref{sec:cr_outcome}; the
estimates in this paragraph were made beforehand and are kept because they set what the regions
could deliver. Once the control regions are opened with beam-on data the same table becomes a genuine
...
\begin{table}[H]
  ...
  \caption{FHC fake-data closure for the twelve proton-tagged observables.
    ...
    not measure the suppression introduced by $A_C$. The third block ($^{\dagger}$) is the two angular observables proposed as secondary scope on 2026-09-18 (\S\ref{sec:wtki_ang}); they are not part of the 2026-09-06 release. \releasestamp}
  \label{tab:wtki_fhc}
  \begin{tabular}{lccc}
\toprule
...
\begin{table}[H]
  \centering
  \caption{RHC fake-data closure for the twelve proton-tagged observables,
    binning and columns as in Table~\ref{tab:wtki_fhc}. All observables close positive with
    the convention-agnostic MC POT normalisation described above. The third block ($^{\dagger}$) is the two angular observables proposed as secondary scope on 2026-09-18 (\S\ref{sec:wtki_ang}); they are not part of the 2026-09-06 release. \releasestamp}
  \label{tab:wtki_rhc}
  \begin{tabular}{lccc}
\toprule
Observable & $\sigma_\mathrm{int}$ & unf./truth & $\chi^2/\mathrm{ndf}$ \\
...
\begin{table}[H]
  \centering
  \caption{Combined FHC$+$RHC fake-data closure for the twelve proton-tagged observables at
    ...
    not required to lie between them; see the discussion below. The third block ($^{\dagger}$) is the two angular observables proposed as secondary scope on 2026-09-18 (\S\ref{sec:wtki_ang}); they are not part of the 2026-09-06 release. \releasestamp}
  \label{tab:wtki_comb}
  \begin{tabular}{lccc}
\toprule
Observable & $\sigma_\mathrm{int}$ & unf./truth & $\chi^2/\mathrm{ndf}$ \\
...
($10\,105$ events; \texttt{bkgfit/protonmatch.py}).
...
Two angular observables were added to the proton-tagged sample on 2026-09-18 and are proposed as
secondary scope; they are not part of the 2026-09-06 release, and the numbers below are fake data
like every other result in this note. Both use the leading proton candidate:
\[
...
\texttt{pi\_pr\_opening\_angle\_\{reco,true\}}, gated on a bit-for-bit comparison of every existing
branch (56/56 files) before the old trees were rotated aside.
...
kind reproduced an earlier release bin by bin to $10^{-9}$ (the comparison record is
in the change log), and repeating that comparison on the release to be unblinded is on
the pre-unblinding list (\S\ref{sec:limitations}).
...
\begin{table}[H]
  ...
  \begin{tabular}{L{3.2cm} c p{8.3cm}}
    ...
    \bottomrule
  \end{tabular}
  \par\smallskip\noindent{\footnotesize $^{\dagger}$ Proposed secondary scope, added 2026-09-18 and
    not part of the 2026-09-06 release: the proton polar angle and the pion--proton opening angle
    (\S\ref{sec:wtki_ang}). $\theta_p$ uses edges derived on the RHC chain, which condition three
    times better than the FHC-derived ones at the same bin count.\par $^{*}$ Numerical plotting boundary only; the upper truth bin is open-ended ($p_\pi>0.205$~GeV/$c$), and the released density over the conventional width $0.795$~GeV/$c$ represents the bin integral (Table~\ref{tab:ppi_partial}).}

\end{table}

The migration-diagonal criterion itself was revisited in 2026-09: relaxing it from $0.68$ to
