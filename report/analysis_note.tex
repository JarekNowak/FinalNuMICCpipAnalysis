...
\begin{titlepage}
  ...
  \begin{abstract}
    ...
    data. After accounting for the FHC Run-1 exposure error, they do not support a class-specific background normalisation correction within the transfer uncertainties; a residual $8$--$16\%$ deficit remains, inside the pre-fit prediction and transfer uncertainties. They did expose a run-level exposure error: the FHC Run-1 beam-on sample was recorded without the open trigger but had been normalised with the open-trigger bookkeeping. The corrected exposure is now adopted and the FHC and combined results are being rebuilt on it, which raises the FHC cross section by $14\%$.
    ...
    pseudo-data thrown from a reweighted interaction model with the nominal response biases no bin
    by more than $0.8\sigma$; unfolding pseudo-data from an independent generator (NuWro) does not
    close, returning $15$--$40\%$ above the NuWro truth in every bin through its larger background
    and higher selection efficiency, a model dependence not yet represented in the covariance.
...
\S\ref{sec:results} is the results section and can be read on its own; \ptref{Sec.}{sec:wtki} gives the
...
\begin{center}\fbox{\parbox{0.94\textwidth}{\textbf{Approval requested.} Approval of the inclusive
CC$1\mu1\pi Xp$ analysis strategy, signal definition, event selection, background treatment,
unfolding procedure, systematic-uncertainty model and primary result set. Signal-region unblinding
is requested only after the FHC Run-1 exposure rebuild, the absolute flux-normalisation
confirmation and the remaining required uncertainty terms are complete and the affected products
regenerated. \textbf{The FHC and combined numbers in this version are provisional}: they predate
the Run-1 exposure correction and will be regenerated, not rescaled.}}\end{center}
...
\begin{itemize}\itemsep1pt \setlength{\parskip}{0pt}
  ...
  \item \textbf{Control regions: opened} on beam-on data under the frozen procedure; they indicate
    no supported class-specific background correction within the transfer uncertainties; an $8$--$16\%$ residual deficit remains within them (\S\ref{sec:cr_outcome}).
  ...
  \item \textbf{Proton-tagged subsample:} a separate measurement with its own note; not part of
    this approval request (\ptref{Sec.}{sec:wtki}).
  ...
  \item \textbf{Validated:} the implementation, the size of the statistical uncertainties, and
    robustness to a reweighted interaction model (FHC and RHC; no bin beyond $0.8\sigma$,
    \S\ref{sec:altmodel}). \textbf{Not passed:} the independent-generator closure with NuWro, which
    comes back $15$--$40\%$ above the NuWro truth in every bin, half from background subtraction
    and half from the efficiency the response assumes (\S\ref{sec:nuwro}). Not tested for the
  ...
  \item \textbf{Before the signal region is opened:} the FHC and combined results must be rebuilt on the corrected FHC Run-1 exposure --- the beam-on sample was recorded without the open trigger, and the correction raises the FHC cross section by $14\%$ --- and the flux normalisation must be confirmed with the flux contact.
\end{itemize}
\end{minipage}}
\end{center}
...
\noindent Throughout this note the pion-momentum bias is the \emph{signed} quantity
$b(p_\pi)=\langle(p_\pi^{\rm reco}-p_\pi^{\rm true})/p_\pi^{\rm true}\rangle$, and it has two regimes
that must not be conflated. \label{sec:ppi_regimes} Below threshold, the small subset of pions that
reconstructs at all is biased \emph{upward} ($+30$ to $+131\%$ in the table): these are the few
tracks long enough to pass, a conditional sample. At the threshold the mass correction removes most
of the hypothesis bias ($-8\%$, with $31\%$ RMS resolution). Above about $0.4\GeVc$ the bias turns
increasingly \emph{negative}, because pions interact before they stop and the visible range is
truncated; the mean reconstructed momentum saturates near $0.34\GeVc$, so the bias reaches $-65\%$
above $0.8\GeVc$. In the $[0.113,0.175]\GeVc$ region the resolution is $51$--$56\%$ with only
$\sim\!3$--$4\%$ reconstructability. Extending the measurement to the $\nu_e$ threshold would thus
...
conversion is a good proxy \emph{for a pion that stops}, and, crucially, the result is a \emph{momentum}, directly comparable to the true $p_\pi$ it is binned against. For a pion that interacts hadronically before stopping the visible range is truncated and the estimator is biased low; that, not the mass hypothesis, is what limits the observable (\S\ref{sec:ppi_regimes}).
...
% =============================================================================
\subsection{Proton-tagged subsample}
\label{sec:selection_1p}
A subsample requiring, in addition, a contained proton candidate with $p_p>0.3\GeVc$ is selected
by the same chain with one further cut. It gives access to hadronic and transverse-imbalance
observables but introduces its own proton-assignment and model-dependence requirements, and it is
documented in a separate note (\ptref{Sec.}{sec:pt_selection}); it is not part of the approval
request made here.
...
(few $\Rightarrow$ prior-biased, many $\Rightarrow$ noisy). It carries no $A_C$ and theory is compared raw. It approximately preserves the total normalisation in the present nominal-model closure; its residual bias is controlled by the iteration count and is biased toward the prior, and can only be evaluated with non-nominal fake data. Its uncertainty here is the data covariance propagated through the iteration at fixed prior.
...
Result integral   & Suppressed, observable-dependent    & Approximately preserved in nominal closure; prior-biased in general \\
...
\begin{figure}[H]
  ...
  \caption{Reconstruction resolution vs.\ true value, selected signal.  Momenta:
    ...
    (\suppref{Sec.}{sec:ppi_bias}): signed mean bias $b=-31\%$, RMS $27\%$. That average is dominated by high momenta; $b$ is only $-2\%$ in the first true bin and reaches $-65\%$ above $0.8$~GeV/$c$, because the reconstructed value saturates near
...
each extraction uses the nominal configured covariance, so its $p$-values are not expected to be uniform and are not a sensitive test
of the extraction --- the pseudo-data and the reference share the systematic sources that the
covariance includes, so they confirm consistency and nothing more; statistical coverage is
evaluated separately with ensembles (\S\ref{sec:validation}). \emph{Third}, an ensemble of
...
\begin{figure}[H]\centering
  ...
  \caption{Differential cross sections $d\sigma/dx$ for the six released distributions on the
    current Poisson-thrown central-value fake data, FHC full exposure: the points are the Wiener-SVD regularised estimator $A_C\,x_{\rm true}$, not an unsmeared estimate of $x_{\rm true}$; unfolded fake
...
should be quoted: the integrals of \ptref{Table}{tab:wtki_fhc}--\ref{P-tab:wtki_comb} are
...
\begin{itemize}\itemsep3pt
  \item \textbf{No supported class-specific correction.} A joint FHC$+$RHC fit of the background
    ...
    background prediction. The fit uses the CC$0\pi$, $\pi^0$ and cosmic-enriched regions only; the
    multi-$\pi$ region, which shares $47$ FHC and $68$ RHC events with the $\pi^0$ region, is
    diagnostic and is not in the fit, so that overlap does not enter the fit covariance.
  \item \textbf{One run's exposure is wrong, and the cause is identified.} The global test passes
    ...
    unaffected because the throws use the same assumed exposure. The beam bookkeeping confirms the non-open-trigger pairing, $2.192\times10^{20}$~POT with $5\,748\,692$ triggers; it is adopted in every configuration, FHC Run~1 alone has been re-thrown at it, and the FHC and combined results are being rebuilt. Until that rebuild lands, every FHC and combined number in this note is the old normalisation (\S\ref{sec:limitations}).
...
\begin{figure}[H]
  ...
  \caption{The opened control regions on beam-on data: FHC (top) and RHC (bottom), data against
    the neutrino-MC $+$ beam-off $+$ dirt prediction in each region. These, not the pseudo-data
    tests that follow, are the evidence for the background model; the FHC Run-1 deficit visible
    here is the exposure error of \S\ref{sec:cr_outcome}. Full per-run ratios, the fitted
    normalisation parameters and their correlations are in the control-sample note.}
  \label{fig:cr_data}
\end{figure}
...
\section{The proton-tagged subsample}
\label{sec:wtki}

A proton-tagged subsample --- the inclusive selection with a reconstructed proton candidate above
$0.3\GeVc$ --- has also been measured: a one-bin total, the calorimetric hadronic-mass proxy, the
four transverse-kinematic-imbalance observables, two proton angles, and two-dimensional pairs. It
is a different measurement, with its own signal definition, proton-assignment requirements and
validation standard, and it is documented in a separate analysis note~\cite{pt_note}. Its products
are not part of the approval request made here. For cross-reference only, its one-bin total is
$0.618\pm0.255$ (FHC), $0.531\pm0.251$ (RHC) and $0.571\pm0.240\times10^{-38}$~cm$^2$/Ar
(combined), on the same blind fake data and the same provisional exposure as the results above.
...
\begin{enumerate}\itemsep3pt
  ...
  \item \textbf{Robustness to model mismatch.} Unfold pseudo-data whose truth differs materially
    ...
    response then introduce. Two tests of increasing strength: with \emph{reweighted} GENIE
    pseudo-data, \emph{passed with a caveat} (\S\ref{sec:altmodel}); with an \emph{independent
    generator}, NuWro, \emph{not passed} --- a coherent $+15$ to $+40\%$ excess, up to $1.9\sigma$
    (\S\ref{sec:nuwro}).
\end{enumerate}
...
To be precise about what this is and is not: the pseudo-data are reconstructed GENIE events reweighted event by event to an alternative parameter set --- signal and background together --- with the same detector simulation, unfolded with the nominal response and compared with the nominal covariance. It is a model-distortion closure, not an independent-generator closure; a NuWro or GiBUU detector-level sample would be the stronger test and remains pending. Two variations were used, in FHC and in RHC. The first is GENIE multisim universe~545, a coherent variation
...
\begin{table}[H]
  \centering
  \caption{Unfolding pseudo-data from an alternative interaction model with the nominal response, FHC (upper block) and RHC (lower).
  ...
  \begin{tabular}{llccc}
    ...
    \midrule
    \multicolumn{5}{l}{\emph{RHC}} \\
    $p_\mu$ & GENIE multisim u545 & $0.843$ & $26.2\%$ & $-0.73\sigma$ \\
            & $\Delta\to N\pi$ angular & $1.049$ & $14.7\%$ & $+0.34\sigma$ \\
    $\cos\theta_\mu$ & GENIE multisim u545 & $0.956$ & $8.7\%$ & $-0.22\sigma$ \\
                     & $\Delta\to N\pi$ angular & $1.013$ & $25.0\%$ & $+0.57\sigma$ \\
    $\cos\theta_\pi$ & GENIE multisim u545 & $0.934$ & $26.4\%$ & $-0.64\sigma$ \\
                     & $\Delta\to N\pi$ angular & $1.008$ & $9.3\%$ & $+0.20\sigma$ \\
    $p_\pi$ (two regions) & GENIE multisim u545 & $0.865$ & $23.2\%$ & $-0.32\sigma$ \\
                          & $\Delta\to N\pi$ angular & $1.036$ & $5.1\%$ & $+0.08\sigma$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
the uncertainties shrank, so this test should be repeated before any future exposure is interpreted.
In RHC the picture is the same with the roles of the edge bins taken by the sparse sixth $p_\mu$ bin
($-0.73\sigma$) and the second $\cos\theta_\pi$ bin ($-0.64\sigma$): no bin beyond $0.73\sigma$, and
the coherent GENIE variation, which lowers the RHC rate, pulls every RHC mean ratio below one
($0.84$--$0.96$) where the angular variation does not. The proton-tagged family has not been tested.

\subsection{Independent-generator closure with NuWro}
\label{sec:nuwro}

The stronger form of the third test uses a different generator end to end. A NuWro overlay of
Run-1 FHC exists with the full detector simulation and its truth branches; it was thrown as fake
data at the corrected FHC exposure and unfolded with the nominal Run-1 GENIE response, background
prediction and covariance. Unlike the reweighted test, everything that can differ between
generators does: the signal kinematics, the background composition and rate, and therefore the
efficiency the response assumes.

\begin{table}[H]
  \centering
  \caption{Unfolding NuWro pseudo-data with the nominal GENIE response, FHC exposure. Columns as in
    Table~\ref{tab:altmodel}; every bin of every observable comes back above the NuWro truth.}
  \label{tab:nuwro}
  \small
  \begin{tabular}{lcccc}
    \toprule
    Observable & bins & mean ratio & range of ratios & largest bias \\
    \midrule
    $p_\mu$ & 7 & $1.145$ & $1.06$--$1.20$ & $+0.78\sigma$ \\
    $\cos\theta_\mu$ & 5 & $1.302$ & $1.27$--$1.33$ & $+1.70\sigma$ \\
    $\cos\theta_\pi$ & 5 & $1.269$ & $1.09$--$1.48$ & $+1.92\sigma$ \\
    $p_\pi$ (two regions) & 2 & $1.411$ & $1.28$--$1.55$ & $+1.64\sigma$ \\
    \bottomrule
  \end{tabular}
\end{table}

This test does not pass. The unfolded result sits above the NuWro truth in every bin of every
observable (Table~\ref{tab:nuwro}), by $15$ to $40\%$ on average and by up to $1.9\sigma$ of the
quoted uncertainty, and the excess is coherent rather than a shape distortion. Its origin is
identifiable, and it has two parts of about equal size. First, NuWro's selected background is
$1.44$ times GENIE's at the same exposure, so subtracting the GENIE background prediction leaves
$15\%$ of the NuWro signal behind. Second, NuWro's signal is selected with $19.1\%$ efficiency
where GENIE's is selected with $16.6\%$ --- its pions are harder and more often contained and
identified --- so the GENIE response under-corrects by a further $15\%$. Their product, $1.32$, is
the excess observed, and it is largest exactly where the background fraction and the efficiency
difference are largest: the near-threshold pion-momentum region and forward $\cos\theta_\pi$.

The two parts have different remedies and should be read separately. The background part is what
the control regions exist to catch: a $44\%$ excess in the CC$0\pi$-dominated background is far
above the $12\%$ threshold of the frozen protocol, so in beam-on data it would be seen and
corrected, and this test therefore overstates it. The efficiency part is not caught by anything in
the chain except a test of this kind, and it is a genuine model dependence of the measurement at
the $15\%$ level --- inside the $25$--$30\%$ flux-and-detector uncertainty of a bin, but not
negligible, and not represented in the covariance. It is the strongest argument in this note for
quoting the interaction-model uncertainty with an explicit generator-spread component rather than
from GENIE reweighting alone, and it is listed as such in \S\ref{sec:limitations}.
...
MCS momentum scale, the beamline geometry (evaluated at $1.6$--$3.3\%$ and bounded there, \S\ref{sec:systematics}), the beam-off gate ratios and the Run-2
...
estimate through the space-charge and recombination models, already cover it. The second
of these is in progress: the momentum of every selected muon is exactly the range value when it is
contained and exactly the MCS value when it exits, so the MCS component can be scaled alone, and
$d\sigma/dp_\mu$ has been re-extracted with that component at $\pm5\%$ and $\pm10\%$ (the binned
quantity only; the precomputed selection flag does not move, which is the limit of this bound). At
$\pm5\%$ the half-difference between the two variations is $3.4\%$ of the bin value on average,
$0.07\sigma$ of the quoted uncertainty, at most $0.19\sigma$ (the top bin); at $\pm10\%$, the size
of the estimator's known bias, it is $6.0\%$ and $0.14\sigma$, at most $0.25\sigma$, with a
$+5\%$ shift of the integral. The term is therefore bounded well inside the quoted uncertainty; it
is not yet in the covariance, and adding it at $\pm5\%$ is the recommended treatment. The
...
\begin{table}[H]
  ...
  \par\smallskip\noindent{\footnotesize $^{\dagger}$ Proposed secondary scope: the proton polar angle and the pion--proton
    opening angle (\ptref{Sec.}{sec:wtki_ang}). $\theta_p$ uses edges derived on the RHC chain, which condition three
...
\begin{table}[H]
  ...
  \begin{tabular}{lrl}
    ...
    Software trigger            & $=1$           & \\
    \midrule
    \multicolumn{3}{l}{\emph{Proton-tagged selection only (\ptref{Sec.}{sec:selection_1p})}} \\
