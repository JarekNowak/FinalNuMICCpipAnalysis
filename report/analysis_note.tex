...
\captionof{table}{Integrated (unfolded) cross section $\sigma_\mathrm{int}$ over the
  ...
  the spread is a smearing effect. The RHC and Combined columns use the corrected
  per-(run,sample) MC POT normalization: an earlier version over-counted the multi-file
  RHC runs (Run~3 split across 5 ntuple files, Run~4 across 3) by scaling each file to
  the full run POT, inflating the simulated background and biasing the RHC/Combined cross
  sections low (FHC, with one MC file per run, was unaffected). With that fixed, the
  RHC integrated cross sections rise by $\sim\!20$--$30\%$ and are now consistent with,
  or slightly above, FHC for every observable --- i.e.\ the previously apparent ``RHC
  below FHC'' deficit was the normalization bug, not $\bar\nu_\mu$ physics. (Values are
  fake-data closures; the signal is $\nu_\mu+\bar\nu_\mu$, so the RHC flux-average is not
  expected to lie far below FHC.)}
\label{tab:sigint_all}
\begin{tabular}{lccc}
\toprule
Observable & FHC & RHC & Combined \\
\midrule
$p_\mu$          & $0.986$ & $0.860$ & $0.818$ \\
$p_\pi$          & $0.976$ & $0.972$ & $0.962$ \\
$\cos\theta_\mu$ & $0.811$ & $0.662$ & $0.702$ \\
$\cos\theta_\pi$ & $0.979$ & $0.805$ & $0.857$ \\
$\theta_{\mu\pi}$& $0.877$ & $0.880$ & $0.922$ \\
\bottomrule
\end{tabular}
\end{center}
