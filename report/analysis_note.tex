...
\begin{table}[htbp]\centering
\caption{Identity closure for every extraction. $\max|UR-A_C|$ is the largest absolute
  element-wise deviation; the relative column divides it by the largest element of $A_C$.
  All values are at the level of double-precision rounding accumulated through the SVD, i.e.\
...
\noindent The worst deviation over all $39$ extractions is $1.4\times10^{-9}$ relative, on
FHC $\delta\alpha_T$; the inclusive observables are all below $7\times10^{-10}$. These are
the rounding errors of the singular-value decomposition and the matrix products that follow
it, not physical effects: they are eleven orders of magnitude below the smallest systematic
uncertainty in the analysis. The unfolding construction is therefore exactly self-consistent,
