...
neutrino direction. Truth-level quantities use the exact per-event neutrino direction,
which reproduces the generator convention exactly; reconstructed quantities use the
fixed target-to-detector axis above, since the per-event direction is not measurable
in data. The residual spread between the two is the physical divergence of the beam
--- neutrinos reaching the detector are produced by decays distributed along the decay
pipe --- and is not recoverable from the interaction vertex: over a $678.8$~m baseline
a $2.5$~m detector subtends only ${\sim}4$~mrad. Measured against the true per-event
direction, the RMS opening angle is $532$~mrad for the detector $z$ axis, $242$~mrad
for the NuMI beamline axis, $178$~mrad for the fixed target-to-detector axis, and
$178$~mrad for a per-event target-to-vertex direction, against a best-possible
$175$~mrad for any fixed axis. A constant axis is therefore as good as anything
available, and the truth/reco difference is a resolution effect absorbed by the
...
target-to-detector axis $(0.4624,\,0.0489,\,0.8853)$; at truth level it is the exact
per-event neutrino direction. The opening angle
...
$56\%$. \textbf{At the current exposure $W_{\pi p}$ does not support a shape measurement} and
should be quoted as an integrated cross section unless the pion momentum improves.
