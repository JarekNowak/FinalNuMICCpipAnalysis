...
\begin{titlepage}
  ...
  \begin{abstract}
    ...
    $0.7$--$1.0\times10^{-38}$~cm$^2$/Ar (Table~\ref{tab:sigint_all}), with RHC
    below FHC as expected for the softer, antineutrino-rich beam.
    %
    ...
    (a consistent $\sim\!20\%$ above Wiener-SVD) confirms that the
...
(Sec.~\ref{sec:crosschecks}), where its flat, iteration-stable integral, a consistent
$\sim20\%$ above Wiener-SVD, confirms that the observable-to-observable spread of the
...
\captionof{table}{Integrated (unfolded) cross section $\sigma_\mathrm{int}$ over the
  ...
  the spread is a smearing effect. RHC generally sits at or below FHC because the RHC
...
\midrule
$p_\mu$          & $0.986$ & $0.858$ & $0.818$ \\
$p_\pi$          & $0.976$ & $0.963$ & $0.962$ \\
...
Six differential observables are measured. The two hadronic-mass variables are the
...
the proton-hypothesis range-based kinetic energy. Given the reduced statistics of the
proton-tagged sample, each observable is measured in four bins, set from the true-signal
distributions (Table~\ref{tab:wtki_binning}).
...
\begin{table}[H]
  \centering
  \caption{Binning of the proton-tagged observables (four bins each), chosen from the
    validated true-signal distributions. The first and last bins are open so the full
    signal phase space is partitioned. $W_{\pi p}$ brackets the $\Delta(1232)$ peak; the
    $\delta\alpha_T$ binning is finer toward $180^\circ$, where final-state interactions
    pile up.}
  ...
  \begin{tabular}{lll}
    ...
    $W_\mathrm{had}$   & calorimetric              & $0,\,1.00,\,1.20,\,1.40,\,1.90$~GeV/$c^2$ \\
    $\delta p_T$       & $|\vec p_T^{\,\mu}+\vec p_T^{\,\mathrm{had}}|$ & $0,\,0.45,\,0.70,\,1.00,\,1.60$~GeV/$c$ \\
    $\delta\alpha_T$   & TKI angle                 & $0,\,90,\,130,\,160,\,180^\circ$ \\
    $\delta\phi_T$     & TKI angle                 & $0,\,50,\,85,\,125,\,180^\circ$ \\
...
integrated-cross-section ratios ($1.02$--$1.22$) reflect the same non-norm-preserving
Wiener-SVD additional smearing $A_C$ seen in the inclusive measurement
(\S\ref{sec:crosschecks}), largest for $W_{\pi p}$ where the sharp $\Delta$ peak is most
smeared. The systematic treatment here includes the flux, cross-section and
...
The RHC and combined measurements are in production and will be added here.

\begin{table}[H]
  \centering
  \caption{FHC fake-data closure for the proton-tagged observables. $\sigma_\mathrm{int}$
    is the integrated (unfolded) cross section over the measured phase space
    ($10^{-38}$~cm$^2$/Ar); ``unf./truth'' is the ratio of integrals; $\chi^2$ is the
    bin-by-bin comparison of unfolded pseudo-data to $A_C$-smeared truth (4 bins). The
    small $\chi^2/\mathrm{ndf}$ validates the chain; the integral ratios are the $A_C$
    smearing effect, not a bias.}
  \label{tab:wtki_fhc}
  \begin{tabular}{lccc}
    \toprule
    ...
    $W_\mathrm{had}$ & $0.59$ & $1.09$ & $0.2/4$ \\
    $\delta p_T$     & $0.56$ & $1.05$ & $0.4/4$ \\
...
Figure~\ref{fig:dsigma_ccpi1p_fhc} shows the six FHC differential cross sections,
overlaid with all four generator predictions (GENIE, GiBUU, NEUT, NuWro) for the same
proton-tagged phase space. The predictions are built at truth level from each
...
\subsection{Multi-pion subselections (\texttt{CC1mu2pi}, \texttt{CC1mu3pi})}
\label{sec:multipion}

Two further subselections extend the analysis to higher charged-pion multiplicities:
$\nu_\mu\,\mathrm{CC}$ with exactly two (\texttt{CC1mu2pi}) or three (\texttt{CC1mu3pi})
charged pions and any number of protons. Both inherit the full inclusive selection,
signal definition and observables from \texttt{CC1mu1piXp}; the charged-pion multiplicity
required by the signal definition and the reco topology is promoted to a virtual so the
derived value ($N=2,3$) takes effect inside the inherited methods.

Naively re-using the single-pion identification gives a prohibitively low efficiency
($4.2\%$ for $2\pi$, $0.8\%$ for $3\pi$), because that identification was tuned for the
$1\pi$ topology: a tight $4$~cm vertex window and TMVA/Bragg/length cuts optimised to
reject protons around a \emph{single} pion. Following the dedicated MicroBooNE multi-pion
study,\footnote{T.\ Mahmud, MSc by Research thesis (\texttt{CC3}$\pi^+$ selection),
Lancaster University; multi-pion analyzer implementation by L.\ Gu.} the multi-pion
selections loosen the pion identification through
five overridable parameters (the inclusive defaults are unchanged, so
Section~\ref{sec:selection} is byte-for-byte identical): the vertex-to-track window is
opened from $4$ to $9.5$~cm; the pion candidate requires only the log-likelihood-ratio
PID ($>0.1$) on top of the common track-score/generation preselection, dropping the
$1\pi$ TMVA/Bragg/length cuts; up to one pion per event is allowed to be uncontained
(the ``maximum uncontainment $=1$'' tolerance); and the two $1\pi$-topology cuts --- the
$\mu$--$\pi$ opening angle (a single-pair cosmic-rejection cut, meaningless with several
pions) and the shower veto (the multi-pion signal is inclusive of extra electromagnetic
activity) --- are not applied. This raises the efficiency to $16.3\%$ ($2\pi$) and
$6.4\%$ ($3\pi$), a factor of $\sim\!4$ and $\sim\!8$ respectively; the number of selected
signal events on the Run-1 FHC $\nu_\mu$ overlay grows from $\sim\!110$ to $530$ ($2\pi$)
and from $\sim\!1$ to $40$ ($3\pi$), the latter turning $3\pi$ from a non-measurement into
a viable flux-averaged total cross section.

Table~\ref{tab:cutflow_multipi} gives the cut-flow. The dominant loss is at the
$N$-pion identification stage (MuonCandidate$\rightarrow$ContainedPions: $62\%\!\to\!22\%$
for $2\pi$, $58\%\!\to\!8\%$ for $3\pi$): reconstructing $N$ pion-like tracks is the
efficiency ceiling, not the downstream topology or quality cuts, which are gentle. The
selections retain the inherited muon-quality, wire-gap, track-multiplicity and
software-trigger requirements that the leaner thesis cut list omits, so they sit below
the thesis efficiencies ($37\%$, $31\%$) at correspondingly higher intrinsic purity. Note
that this cut-flow is the $\nu_\mu$ overlay only (Run-1 FHC): beam-off cosmics and dirt
are not included, so the quoted purity is an MC-intrinsic upper bound and will fall once
the EXT sample --- the dominant background at these multiplicities --- is folded in.

\begin{table}[H]
  \centering
  \caption{Multi-pion cut-flow yields (Run-1 FHC $\nu_\mu$ overlay, POT-weighted; beam-off
    and dirt not included). Stages follow the inherited inclusive selection with the
    multi-pion pion identification (Section~\ref{sec:multipion}); the shower veto and
    opening-angle cut are not applied. ``Total'' is signal $+$ MC background; Eff.\ is the
    cumulative signal efficiency relative to the generated signal (None stage); Pur.\ $=$
    Signal/Total (MC-intrinsic --- an upper bound; the real-sample purity is lower once
    EXT cosmics are included).}
  \label{tab:cutflow_multipi}
  \small
  \begin{tabular}{lrrrrr}
    ...
    None            & 759\,496 & 3\,251 & 756\,245 & 100.0 & 0.4 \\
    InFiducialVol   & 177\,783 & 2\,751 & 175\,033 & 84.6 & 1.5 \\
    Topological     & 83\,662 & 2\,259 & 81\,403 & 69.5 & 2.7 \\
    MuonCandidate   & 58\,791 & 2\,014 & 56\,776 & 62.0 & 3.4 \\
    ContainedPions  & 3\,372 & 719 & 2\,653 & 22.1 & 21.3 \\
    MuonIn3Planes   & 3\,181 & 681 & 2\,500 & 21.0 & 21.4 \\
    PionIn3Planes   & 2\,786 & 600 & 2\,186 & 18.5 & 21.5 \\
    \textbf{Final}  & \textbf{2\,500} & \textbf{530} & \textbf{1\,970} & \textbf{16.3} & \textbf{21.2} \\
    \midrule
    \multicolumn{6}{l}{\textit{\texttt{CC1mu3pi} --- exactly 3 charged pions}} \\
    None            & 759\,496 & 625 & 758\,871 & 100.0 & 0.08 \\
    InFiducialVol   & 177\,783 & 518 & 177\,265 & 82.9 & 0.3 \\
    Topological     & 83\,662 & 425 & 83\,237 & 68.0 & 0.5 \\
    MuonCandidate   & 58\,791 & 365 & 58\,426 & 58.4 & 0.6 \\
    ContainedPions  & 447 & 50 & 397 & 8.0 & 11.2 \\
    MuonIn3Planes   & 415 & 48 & 367 & 7.7 & 11.6 \\
    PionIn3Planes   & 384 & 45 & 339 & 7.2 & 11.7 \\
    \textbf{Final}  & \textbf{344} & \textbf{40} & \textbf{304} & \textbf{6.4} & \textbf{11.6} \\
    \bottomrule
  \end{tabular}
\end{table}

Feasibility: with $\sim\!530$ selected signal events at full exposure the $2\pi$ channel
can support a flux-averaged total cross section and possibly a coarse ($2$--$3$ bin)
differential; the $3\pi$ channel ($\sim\!40$ events) is a total-cross-section
measurement. A dedicated background treatment (EXT cosmics dominate) and the extension to
the remaining runs and RHC are left as follow-up.
