...
few-per-cent sensitivity to the signal normalisation: the CC$0\pi$ region holds $1\,124$
signal events ($9\%$ of its neutrino MC), so a $30\%$ signal excess would appear as a
$2.8\%$ excess in a region of $1\%$ statistical precision, invisible to the
full-covariance pull but visible at $\sim3\sigma$ to the statistical-plus-detector pull;
the response tree holds the signal at its central value and records such an excursion
...
\begin{enumerate}\itemsep2pt
  ...
  \item \textbf{Permitted responses and the rule for choosing between them.} For a region
    ...
    target-class purity (its share of the neutrino-MC prediction in the region; the target
    classes are $\nu_\mu$CC~$0\pi$ for CC$0\pi$ and CC$+$NC~$\pi^0$ for $\pi^0$, ratified
    2026-09-06) exceeds $50\%$, a one-parameter normalisation fit of that class is
