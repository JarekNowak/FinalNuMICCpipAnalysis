...
\begin{enumerate}[itemsep=3pt]
  \item Two two-dimensional pairs are worth proposing as released measurements, in the combined
    configuration only: $\theta_p\times\delta p_T$ and $\cos\theta_\pi\times\cos\theta_\mu$
    (Table~\ref{tab:2dextract}, Figs.~\ref{fig:2dslices}--\ref{fig:2dmap}). Both need generator predictions checked against the flattened bin
    order and a decision on whether a combined-only result is acceptable, the released differential
    results being quoted per beam mode as well.
  \item $\theta_p$ in RHC is rank-deficient, and \S\ref{sec:thetap_rhc} shows that coarsening does
    not rescue it ($0.002\to0.050$ at three bins): it can only be quoted for FHC and the combined
    configuration. Open instead is which edges $\theta_p$ should use at all, the RHC-derived scheme
    being three to four times better conditioned in both usable configurations than the released one.
