...
% Every section is tagged by which measurement it serves, as the reviewer asked: shared
% infrastructure, the inclusive measurement, or the proton-tagged one.
\newcommand{\scopetag}[1]{\hfill{\normalfont\small\textsc{[#1]}}}
\begin{document}
\begin{titlepage}
\centering
\vspace*{2cm}
...
\label{sec:supp_intro}

The analysis note says what was measured and why the choices were made. This supplement holds the
studies behind those choices, so that a reader who wants to check one of them can, without the
note having to carry them. Nothing here is needed to follow the note, and every study here is
pointed at from the place in the note where its conclusion is used.

It is organised by question rather than by when the work was done.
\begin{itemize}\itemsep2pt
  \item \textbf{Estimators and binning}: why the pion momentum is reported in two regions and what
    was tried to do better; how every binning was chosen and re-tested; the proton-tagged
    observables and their additional smearing; the muon angle in its two parameterisations.
  \item \textbf{Validation of the extraction}: the identity closure, what the closure $\chi^2$
    does and does not test, the ensemble study, and the consistency between configurations and
    with the sibling $\nu_e$ measurement.
  \item \textbf{Control regions}: the definitions, the frozen procedure under which they were
    opened, the transfer factors and their uncertainties.
  \item \textbf{Selection and infrastructure}: the selection optimisation, cuts considered and
    rejected, and the automated guards on the pipeline.
  \item \textbf{Physics background}: the production mechanisms, nuclear effects and prior
    measurements, at greater length than the note allows.
\end{itemize}
Terms of art are defined in the Glossary at the end, which is shared with the note.

\part{Estimators and binning}
\section[Pion momentum: the range estimator and why it fails]{Pion momentum: the range estimator and why it fails\scopetag{inclusive}}
\label{sec:supp_ppi}
...
\subsubsection{Two separate effects, only one of them corrected.}
...
\subsubsection{The reconstructed momentum saturates.}
...
\subsubsection{What that does to the response matrix.}
...
\subsubsection{How the BNB CC1$\pi^\pm$ analysis handles the same problem.}
...
\subsubsection{Consequences, and what would improve it.}
...
\subsubsection{Wiggliness works, but with the opposite sign to the naive expectation.}
...
\subsubsection{Closure.} The two-bin scheme is carried through the full chain, universe
...
\caption{Two-bin $p_\pi$ result for all three beam configurations, $d\sigma/dp_\pi$ in
  ...
  Generated from the release (\texttt{report/tools/ppi2bin\_tables.py}: closure sidecars and
...
Figure~\ref{fig:ppi2bin_AC} shows $A_C$ for all three configurations.
...
\subsubsection{As a classifier it reproduces the BNB benchmark.} ROC integral $0.758$. At
...
\subsubsection{But it does not fix the binning, and cannot.} Table~\ref{tab:golden_compare}
...
\subsubsection{These diagonals are measured on a different sample from the analysis ones, and
...
\subsubsection{Multiple Coulomb scattering was tried as a pion momentum estimator, and as a
...
\subsubsection{A caveat on the tighter working points.} The $>0.15$ row looks better than
...
\subsubsection{Why, and what is left.} Golden pions still reconstruct at
...
\subsubsection{Conclusion.} The comparison is worth stating plainly rather than filing as a
...
\subsubsection{Scope of $p_\pi$.} Both candidate routes to a finer $p_\pi$ measurement were
...
from it; what it cannot support is a five-bin \emph{shape} measurement.
...
\subsubsection{The proton-tagged route.} The other candidate route to a sharper $p_\pi$
...
\caption{Two-bin $p_\pi$ on the proton-tagged \texttt{CC1mu1pi1p} selection, all three beam
  ...
  generator rows appear. Regenerated from the release
...
\subsubsection{The least-populated bin.} The RHC first bin unfolds to $0.37$ times its
...
\section[Rejected: extending the pion threshold below 0.175\,GeV/\textit{c}]{Rejected: extending the pion threshold below 0.175\,GeV/\textit{c}\scopetag{inclusive}}
...
\section[Binning Optimisation, Systematic Breakdown, and Threshold Validation]{Binning Optimisation, Systematic Breakdown, and Threshold Validation\scopetag{inclusive}}
\label{sec:binopt}
% =============================================================================
...
candidates that passed were built and unfolded in full (universes, covariance, closure) and compared with the released extraction on the $A_C$ row
sums, the $A_C$ conditioning ($s_{\min}/s_1$), the data-statistical term, the per-bin
total uncertainties and the closure ratios.

\begin{center}\small
\captionof{table}{(Binning re-check on the release preceding the $\cos\theta_\pi$ decision: ``released'' means the binning in use when the test was run; the forward-split five-bin $\cos\theta_\pi$ row is the released binning.) Pre-screen: smallest column-normalised migration diagonal over the true
...
\begin{center}\small
\captionof{table}{(Binning re-check on the release preceding the $\cos\theta_\pi$ decision: ``released'' means the binning in use when the test was run; the forward-split five-bin $\cos\theta_\pi$ row is the released binning.) Full extraction of the candidates that passed the pre-screen, FHC / RHC:
...
observables were re-tested on the maximin scan and against a relaxed $0.50$ diagonal
...
\section[Hadronic mass, model comparison, and estimator surveys]{Hadronic mass, model comparison, and estimator surveys\scopetag{proton-tagged}}
\label{sec:supp_w}
...
measurement. A finer edge scan of the six-bin $W_\mathrm{had}$ has not been run. The two-bin alternative was extracted and is better conditioned ($0.58/0.54$) but was not adopted, because it discards the shape the observable is measured for; the six-bin scheme is retained with that caveat.
...
\begin{table}[H]
  ...
  \caption{Wiener-SVD vs.\ D'Agostini (iterative Bayesian, 4~iterations) integrated
    ...
    $\cos\theta_\mu$, $\cos\theta_\pi$, $\theta_{\mu\pi}$; $10^{-38}$~cm$^2$/Ar), on the current release. D'Agostini carries no $A_C$ smearing, so it recovers a
...
\section[Proton-tagged selection, binning and additional smearing]{Proton-tagged selection, binning and additional smearing\scopetag{proton-tagged}}
...
\begin{table}[H]
  ...
  \begin{tabular}{llcl}
    ...
    \bottomrule
  \end{tabular}
  \par\smallskip\noindent{\footnotesize $^{\dagger}$ Proposed secondary scope. $\theta_p$'s edges are derived on the RHC chain.}
\end{table}



\subsection{Reconstruction of the proton-tagged observables from input quantities}
\label{sec:wtki_reco}
...
track ends inside the containment volume (\noteref{Sec.}{sec:fv_where}), and
...
(\noteref{Sec.}{sec:selection}); the same two-bin configuration formula is used. The proton momentum follows from its
...
$M_{A'}=M_A-m_n+E_b$ and $E_b=24.78$~MeV; $z$ is the beam axis (\noteref{Sec.}{sec:beamdir}).
...
(Sec.~\ref{sec:acwtki}), so the comparison is like for like; a truth-level closure of
...
Sec.~\ref{N-sec:limitations}.



\paragraph{Two exposure conventions in the processed files.}
...
observable is enhanced by $A_C$ (in the released configurations every prediction-dependent integral factor is below unity --- an empirical property of these extractions, not a mathematical requirement on $A_C$, which is a regularisation operator and can suppress or enhance; an amplification
...
\begin{table}[H]
  ...
  \caption{Mean $A_C$ row sum for the proton-tagged observables (FHC), from the released
    matrices \texttt{A\_C\_1p\_FHC5\_*.tsv}, release; equal to the ratio of the
    $A_C$-smeared to the truth-level integral for a flat prediction. The inclusive
    observables are given for scale.}
  \label{tab:acwtki}
  \par\smallskip\noindent{\footnotesize $^{\dagger}$ From the current extraction; these observables are not yet in the data release.}
  \begin{tabular}{lc}
      \toprule
...
versus \texorpdfstring{$\theta_\mu$}{theta mu} forensics]{Angular observables: the \texorpdfstring{$\cos\theta_\mu$}{cos(theta mu)}
versus \texorpdfstring{$\theta_\mu$}{theta mu} forensics\scopetag{inclusive}}
\label{sec:supp_ang}
...
\caption{$A_C$ conditioning $s_2/s_1$ with the uniform-grid stencil (``uniform'') and
  with the width-aware operator (``width-aware''), measured on one fake-data pass at the binnings in use when the test was run (the $\cos\theta_\pi$ row is a five-bin scheme since replaced);
...
\begin{table}[H]
  ...
  \caption{Total flux-averaged cross section from the one-bin extraction
    ...
    \noteref{Sec.}{sec:limitations}). \texttt{data\_release/total\_xsec.tsv}. Values in
...
\captionof{table}{FHC (\{Run~1,2,4,5\}) systematic breakdown by source, bin-averaged
  fractional uncertainty (\%), regenerated from the systematics dumps of the release (\texttt{logs/systdump}); ``Prediction total'' excludes
  the data statistics.}
...
\part{Validation of the extraction}
\section[Identity closure: unfolding the prediction with itself]{Identity closure: unfolding the prediction with itself\scopetag{shared}}
\label{sec:identity}
...
% =============================================================================
\section[Why the results are presented as they are]{Why the results are presented as they are\scopetag{inclusive}}
\label{sec:supp_caveats}

\subsection{The two-bin \texorpdfstring{$p_\pi$}{p pi} result}
...
$91\%$ and $76\%$ (column-normalised $M$, \noteref{Sec.}{sec:unfolding_resp}) and are sound,
...
appears in the data release and in \noteref{Appendix}{app:binning} is only the finite boundary
...
\captionof{table}{Two-bin $p_\pi$ result as bin-integrated partial cross sections
  ...
  released covariance (the same fractional uncertainties as \noteref{Tables}{tab:fhc}--\ref{N-tab:comb}).
...
\subsection{What the closure \texorpdfstring{$\chi^2$}{chi-squared} values mean}
...
release; the released closure values are those of \noteref{Tables}{tab:fhc}--\ref{N-tab:comb},
...
\section[Consistency between configurations and with the sibling $\nu_e$ measurement]{Consistency between configurations and with the sibling $\nu_e$ measurement\scopetag{inclusive}}
...
\part{Control regions}
\section[Control regions: the opening request and its sign-off]{Control regions: the opening request and its sign-off\scopetag{inclusive}}
\label{sec:request}
% =============================================================================
...
\noteref{Sec.}{sec:sidebands}; the numbers are those of the frozen release identified in the box
below.

\medskip\noindent\fbox{\parbox{0.97\linewidth}{\textbf{Analysis state.} Repository tag \texttt{cr-freeze-2026-09-06};
...
\begin{table}[H]
  ...
  \caption{Control-region definitions in terms of the selection stages. The
    ...
    (Cuts~1--4 of \noteref{Sec.}{sec:selection}); \textit{shower} is the shower veto (Cut~8),
...
Model-distortion fake-data closure (reweighted GENIE, FHC) & done, \noteref{Sec.}{sec:validation} & \\
Independent-generator (NuWro or GiBUU) fake-data closure & pending: the existing NuWro overlay failed validation against the standalone prediction and is not used, \noteref{Sec.}{sec:limitations} & \\
Flux constants confirmed as PPFX-corrected central values & pending, \noteref{Sec.}{sec:flux_status} & flux contact \\
MCS muon-momentum uncertainty bounded or propagated & pending, \noteref{Sec.}{sec:limitations} & \\
...
\noteref{Table}{tab:sideband_yields} gives the expected composition per region and beam. The
...
\section[Control-region machinery]{Control-region machinery\scopetag{inclusive}}
\label{sec:cr_machinery}
...
\noteref{Table}{tab:sidebands} compares the neutrino-MC throw with the neutrino MC alone. To
...
\begin{table}[H]
  ...
  \caption{Normalisation uncertainty (\%) of the neutrino-MC prediction in the signal region
    ...
    $\nu$-MC yields apply the safe-weight rule of \noteref{Eq.}{eq:safeweight}, as do
    \noteref{Tables}{tab:cutflow} and~\ref{N-tab:sideband_yields} and the pseudo-data throws;
...
for below-threshold pions (\noteref{Table}{tab:pithresh}). And the CC$0\pi$ region constrains
...
The frozen $\pi^0$ definition drops the final multiplicity cut (Sec.~\ref{sec:request}). The
...
\noteref{Table}{tab:sideband_yields} and the uncertainties of \S\ref{sec:cr_transfer}. Table~\ref{tab:cr_pi0_final}
...
\begin{table}[H]
  ...
  \caption{The $\pi^0$ control region without (frozen) and with the final multiplicity
    ...
    class, the leverage denominator of Sec.~\ref{sec:request}; the CC$0\pi$ line is part of it.}
...
topologies the multi-shower argument of Sec.~\ref{sec:request} anticipated, and they are
...
\begin{enumerate}\itemsep2pt
  ...
  \item \textbf{Statistics.} Three quantities are computed for each region and beam
    ...
    not make the test powerless, as the closure $\chi^2$ of \noteref{Sec.}{sec:chi2} shows
  ...
  \item \textbf{Permitted responses and the rule for choosing between them.} The response
    ...
    classes $\nu_\mu$CC~$0\pi$ and CC$+$NC~$\pi^0$, adopted, ratification
    requested in Sec.~\ref{sec:request}) and only if the
    ...
    (Sec.~\ref{sec:request}), so its eligibility is \emph{not} pre-authorised by this
    ...
    unchanged (the $1+B/S$ coherence of \noteref{Sec.}{sec:fluxanatomy} is retained, with $B$
    ...
    and $0.13$ in CC$0\pi$; Sec.~\ref{sec:request}).
    ...
    The response letters used here and in Sec.~\ref{sec:request} are (a)~no action,
...
The request of Sec.~\ref{sec:request} covers the four inclusive regions. The proton-tagged
...
\subsection[Control regions for the proton-tagged subsample]{Control regions for the proton-tagged subsample\scopetag{proton-tagged}}
\label{sec:sidebands_1p}

The proton-tagged selection inherits its cut logic from the inclusive selection, and with it
the four sideband definitions of \noteref{Sec.}{sec:sidebands}: the reprocessed proton-tagged
...
\part{Selection, reconstruction and infrastructure}
\section[Selection studies and supporting material]{Selection studies and supporting material\scopetag{inclusive}}
...
\section[Analysis Pipeline and Automated Guards]{Analysis Pipeline and Automated Guards\scopetag{shared}}
\label{sec:pipeline}
% =============================================================================
...
the pre-unblinding list (\noteref{Sec.}{sec:limitations}).




\part{Physics background}
\section[Physics background and prior measurements (extended)]{Physics background and prior measurements (extended)\scopetag{shared}}
