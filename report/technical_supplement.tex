...
it, at the uncertainty band then in use; with the current one-bin band these read $0.5\sigma$ and
...
\begin{table}[H]
  ...
  \caption{Proton-tagged (\texttt{CC1mu1pi1p}) cut-flow yields at full exposure for
    ...
    (\texttt{reprocess\_cutflow\_cf\_1p.sh}, 2026-09-05); EXT from the per-run beam-off
...
\begin{table}[H]
  \centering
  \caption{Released binning of the six W/TKI proton-tagged observables, read from the
  ...
  \begin{tabular}{lll}
    \toprule
    Observable & Definition & Released bin edges \\
    \midrule
    $W_{\pi p}$        & $M(\pi p)$, direct        & $1.08,\,1.19,\,1.23,\,1.27,\,1.34,\,1.47,\,2.9$~GeV/$c^2$ (six bins; shape withdrawn) \\
    $W_\mathrm{had}$   & calorimetric              & $0,\,0.78,\,1.08,\,1.21,\,1.31,\,1.44,\,2.74$~GeV/$c^2$ (six bins) \\
    $\delta p_T$       & $|\vec p_T^{\,\mu}+\vec p_T^{\,\mathrm{had}}|$ & $0,\,0.3,\,2.5$~GeV/$c$ (two bins) \\
    $\delta\alpha_T$   & TKI angle                 & $0,\,120,\,180^\circ$ (two bins) \\
    $\delta\phi_T$     & TKI angle                 & $0,\,50,\,180^\circ$ (two bins) \\
    $p_n$              & inferred nucleon momentum & $0,\,0.375,\,2$~GeV/$c$ (two bins) \\
    \bottomrule
  \end{tabular}
\end{table}
...
variations included, 2026-09-04 release). It closes against the $A_C$-smeared truth, but that
...
$0.5$--$0.8\sigma$ low, while GiBUU, NEUT and NuWro lie within $0.8\sigma$. Earlier
versions of this table used the cut-and-count central value with the prediction total of
the released $p_\mu$ extraction as its uncertainty; that borrowed systematic carried the
regularisation amplification and is withdrawn (analysis note, change log).
...
\begin{table}[H]
  ...
  \caption{Total flux-averaged cross section from the one-bin extraction
    ...
    generator predictions. The measurement uncertainty is the full one-bin covariance
    (flux, cross section, re-interaction, detector, POT, targets and statistics). Values in
...
\captionof{table}{FHC (\{Run~1,2,4,5\}) systematic breakdown by source, bin-averaged
  fractional uncertainty (\%), regenerated from the systematics dumps of the 2026-09-04
...
\subsection{Re-check of every released binning on the 2026-09-04 release}
\label{sec:binning_recheck}
The binnings above were fixed on an earlier release, and the coarsening of $\cos\theta_\pi$
($5\to4$) and $\theta_{\mu\pi}$ ($7\to5$) was driven by $A_C$ smearing on that release.
Since then the per-run normalisation, the weights, the beam-off treatment, the fake-data
throw and the native RHC detector variations all changed, so every released binning was
re-tested on the current release for whether finer or more bins are admissible. Two
stages were used. First a pre-screen on the processed files
...
\begin{center}\small
\captionof{table}{Pre-screen: smallest column-normalised migration diagonal over the true
...
\begin{center}\small
\captionof{table}{Full extraction of the candidates that passed the pre-screen, FHC / RHC:
...
\midrule
$\cos\theta_\pi$ released 4 & $0.71$--$0.82$ / $0.79$--$0.86$ & $0.27$ / $0.24$ & $12.0$ / $11.5\%$ & $30$--$49$ / $32$--$55\%$ & $0.95$--$1.09$ / $0.94$--$1.07$ \\
...
\midrule
$\theta_{\mu\pi}$ released 5 & $0.52$--$1.03$ / $0.58$--$1.15$ & $0.008$ / $0.002$ & $8.2$ / $8.4\%$ & $35$--$40$ / $30$--$39\%$ & $0.95$--$1.06$ / $1.03$--$1.08$ \\
...
$0.27$ to $0.46$ in FHC, $0.24$ to $0.38$ in RHC) while closing to $0.89$--$1.07$. It is
admissible and is the only change this re-check supports; adopting it would require the
generator predictions to be regenerated at the new edges and the three $\cos\theta_\pi$
extractions and their control-region binning to be re-released, and is offered as a
decision rather than made here. The released binnings of the proton-tagged observables,
which are set by the same resolutions on a sample a third the size, were not re-tested.
...
correction of 2026-09-02. The one-bin totals of the current release
...
The reported cross sections are the \textsc{XSecAnalyzer} framework extraction on the
Poisson-thrown central-value fake data of the analysis note (this section's own
comparisons were made on the earlier GENIE detector-variation-CV fake data). The fully
independent custom pipeline (NuWro fake data) reproduced the framework result of that era
and provided the end-to-end cross-check; it has not been re-run on the released binnings
and weighting (analysis note, Limitations). Reaching the
