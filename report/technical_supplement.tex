...
consistency where current results are duplicated (\texttt{report/tools/check\_tables.py},
...
trained and found insufficient (\S\ref{sec:ppi_bias}).]}
...
amounts, $0.63$--$0.79$ (Table~\ref{tab:acwtki}): the two-bin TKI variables and
$W_{\pi p}$ sit at $0.70$--$0.79$ and the six-bin $W_\mathrm{had}$, the most affected, at
$0.63$. Sharpness alone does not drive the suppression: $W_{\pi p}$ is the sharpest
...
\begin{table}[H]
  ...
  \caption{Total flux-averaged cross section from the one-bin extraction
    ...
    MCS-momentum and beamline-geometry terms are not propagated,
...
The detector systematic is anomalously large for COMB. For $p_\mu$ it is $23.1\%$ of
