...
The two documents were generated from a common source and are checked for numerical
consistency where current results are duplicated (\texttt{report/check\_tables.py}, \texttt{xsec\_analyzer/check\_staleness.py},
\texttt{check\_staleness.py}); that is a check, not a guarantee, and the change log records
what each round of review found.
...
treatment is worth reproducing here, because it is a solved problem there and, at the
time of this study, an open one here.
...
regularisation than any other observable (the pre-fix $A_C$ survey of
Table~\ref{tab:ac_survey}, superseded in its numbers, showed the momentum observables
retaining the least model discrimination through $A_C$). With $6\%$
...
before the $p_\pi$ spectrum is quoted as a shape measurement. \emph{[Subsequently done: the
two-bin scheme was adopted (\S\ref{sec:ppi2bin_adopted}) and the golden-pion classifier was
trained and found insufficient (\S\ref{sec:golden_bdt}).]}
...
total-cross-section comparison (now the one-bin extraction of Table~\ref{tab:total_xsec}) GENIE went from $1.0\sigma$ to
...
$0.7\sigma$. Had the $p_\pi$ prediction alone been used, the $8\%$ deficit would have appeared as a
...
chain, universe construction, Wiener--SVD unfolding, systematics, and closed cleanly on
the release of the time (2026-08-17; the current values are those of
Table~\ref{tab:ppi2bin} below and of the analysis note):
...
\caption{Two-bin $p_\pi$ result for all three beam configurations, $d\sigma/dp_\pi$ in
  ...
  Generator predictions use the corrected (untruncated) FTE files. Regenerated from the
  2026-09-06 release (\texttt{report/tools/ppi2bin\_tables.py}: closure sidecars and
...
normalisation offset associated with how hard $A_C$ smears; an earlier draft quoted a
ratio of $1.13$ here, which belonged to a superseded fake-data pass and is withdrawn.
...
\paragraph{The least-populated bin.} On the release of 2026-08-17 the RHC first bin
unfolded to $1.88$ times its $A_C$-smeared truth; on the current release, after the
re-throw, it sits at $0.37$ times it ($0.386\pm0.796$ against $1.056$), with the closure
$\chi^2$ of $1.21/2$ unremarkable either way. That the same bin swings from $+1.1\sigma$
to $-0.8\sigma$ between two independent throws is the honest description of it: the bin
holds too few events to say anything, and any real-data value in it should be read with
its uncertainty and not its central value.
...
proton momenta with equal-population bins that track the $\Delta(1232)$ peak. No observable
is enhanced by $A_C$ any more: the pre-fix set, in which $\delta p_T$ was suppressed to
$0.43$ and $p_n$ amplified to $1.7$, was an artefact of the uniform-grid regularisation
(footnote to the table), and the caveat that once attached to those two panels no longer
applies. The suppression is applied identically to data, tune and generators, so model
...
\begin{table}[H]
  ...
  \begin{tabular}{lc}
      ...
      \multicolumn{2}{p{0.72\textwidth}}{\emph{Computed from the released matrices
        ...
        removal of the duplicated $A_C$.  The values span $0.61$--$0.79$,
        a range of $0.18$.  The pre-fix set ($0.88$, $0.78$, $0.63$,
...
\emph{[SUPERSEDED --- this paragraph and the next were written before the beam-axis
correction and the removal of the duplicated $A_C$; the $\chi^2$ they discuss compare
observables computed about the wrong axis, and the $\theta_\mu$ factor-9 reduction was the
double-$A_C$ bug (change log). Retained as record only; current comparisons are the
proton-tagged tables of the analysis note.]} This argument was made with the pre-fix $A_C$ and does not survive. With the
...
artefact explanation was withdrawn on that pass.
...
quoted for $W_{\pi p}$ on that pass reflects a binning that cannot resolve the differences
between generators, not agreement between them. The same test should be applied to $W_\mathrm{had}$, whose upper bins are also narrower
...
large once systematics are folded in; the $p_\pi$ first bin, with $469$ events, carried
$42$--$53\%$ on that pass and $77\%$ on the current release (Table~\ref{tab:ppi2bin_1p}). Second, if that proves too sparse in practice, $1.17$ is the natural
...
\noindent\emph{(Matrices of the 2026-08 pass in which the $W_{\pi p}$ two-bin scheme was run,
written with true bins as rows; superseded. The released inclusive $p_\pi$ matrix is that of
Fig.~\ref{fig:ppi2bin_AC}.)}
\begin{center}\small
\begin{tabular}{lcc}
\toprule
...
variations included, 2026-09-06 release). It closes against the $A_C$-smeared truth, but that
...
and every generator, they are compared on equal footing; the $2$--$3\times$ deficit of the
generators reported in earlier drafts was the duplicated $A_C$ (change log), not a property
of the comparison.
...
flat yet was \emph{enhanced} to $1.71$; an enhancement no peak argument can produce. The retraction stands; the width-aware $A_C$ removed the enhancement and narrowed the
proton-tagged factors to $0.61$--$0.79$ (Table~\ref{tab:acwtki}), while the inclusive
$\cos\theta_\mu$ factors remain far below the rest ($0.17$--$0.39$ against $0.66$--$0.91$
for $p_\mu$), so the suppression is set by the response conditioning, not by shape. Within
...
tune) and were suppressed the \emph{most} ($0.37$ and $0.29$, against $0.67$) \emph{[pre-fix,
2026-08-18 pass with the duplicated $A_C$; on the current release no smeared generator
bin is negative in $\cos\theta_\mu$ and NEUT and NuWro sit above the tune in every bin]}.
What drove their suppression on that pass was that $A_C$ pushed their backward bins negative
...
\begin{table}[htbp]\centering
\caption{(2026-08-18 pass, measured immediately after the fix; the $\cos\theta_\pi$ row is the
  five-bin scheme of that time; current conditioning values are derivable from the released
  matrices.) $A_C$ conditioning $s_2/s_1$ before and after the regularisation fix, with the
...
\paragraph{$\theta_\mu$ is reinstated.} The fake data are Poisson throws from the
MicroBooNE tune, so the tune is the model that \emph{ought} to fit best. Before the fix it
did not stand out, because every model had been mapped onto the same curve; after it, it
does, though less dramatically than the first post-fix numbers suggested, which still
carried the duplicated $A_C$ (change log). On the current release:

\begin{center}\small
\begin{tabular}{lcc}
\toprule
FHC $\theta_\mu$, $\chi^2/5$ bins & before the fix (rank-1) & current release \\\midrule
MicroBooNE tune & $3.07$ ($p=0.69$) & $\mathbf{2.14}$ ($p=0.83$) \\
GENIE           & $4.73$ ($p=0.45$) & $5.55$ ($p=0.35$) \\
GiBUU           & $3.03$ ($p=0.70$) & $6.07$ ($p=0.30$) \\
NuWro           & $2.90$ ($p=0.72$) & $9.10$ ($p=0.11$) \\
NEUT            & $2.82$ ($p=0.73$) & $8.46$ ($p=0.13$) \\
\bottomrule
\end{tabular}
\end{center}

\noindent The closure is $1.52/5$ ($p=0.91$). The $\theta_\mu$ measurement is therefore no
longer withdrawn: the tune fits best and the generators are separated from it, though at
this exposure only at the $1$--$2\sigma$ level. What must be withdrawn instead is the earlier
\emph{explanation} that $\theta_\mu$ ``fixes'' the $\cos\theta_\mu$ tension; both
parameterisations are now properly conditioned and should be read together (the
$\theta_\mu$ $\chi^2$ are systematically the larger, analysis note
...
\begin{table}[htbp]\centering
\caption{\textbf{[SUPERSEDED --- 2026-08 survey, before the width-aware regularisation.]}
Retention of model discrimination through $A_C$. ``Before'' and ``after'' are the
...
\emph{Everything from here to Table~\ref{tab:theta_vs_costheta} was written on the pre-fix
survey of Table~\ref{tab:ac_survey} and is superseded in its numbers and its conclusions;
see the correction above and the current release values in the analysis note.} As measured then, the collapse looked
...
of unphysical negative excursions. Neither was a clean muon-angle measurement on that
pass; after the fix both are released, with the caveats of \noteref{Table}{tab:chi2_theta}.

\begin{table}[H]
  \centering
  \caption{\textbf{[SUPERSEDED --- 2026-08-18 pass, retained for the record.]} Integrated
    cross section ($10^{-38}$~cm$^2$/Ar) for the unfolded fake data, the MicroBooNE tune and
    the four generators, in $\cos\theta_\mu$ and in $\theta_\mu$, as measured before the
    regularisation fix, the removal of the duplicated $A_C$ (which is why the generator
    columns sit far below the tune), the beam-off corrections and the re-throw. On the
    current release the unfolded integrals are $0.754$ ($\cos\theta_\mu$) and $0.960$
    ($\theta_\mu$) for FHC and $0.733$ and $0.805$ for RHC (analysis note,
    \noteref{Table}{tab:sigint_all}), and the generator comparison is
    \noteref{Table}{tab:chi2_theta}. The FHC $\theta_\mu$ row of this table was made under
    a rank-1 $A_C$ and carries no shape information.}
...
\begin{figure}[H]
  ...
  \caption{Muon-angle differential cross section in $\theta_\mu$ (left) and
    ...
    backward bins $A_C$ drives negative. \textbf{Historical figure (2026-08-18, not
    regenerated)}: every panel predates the regularisation fix, the removal of the
    duplicated $A_C$, the beam-off corrections and the fake-data re-throw; the FHC
    $\theta_\mu$ panel in particular was made under a rank-1 $A_C$. The current
    distributions are Figs.~\ref{N-fig:dsigma_current}--\ref{N-fig:dsigma_comb} of the analysis note.}
...
This section records the 2026-08 binning optimisation on which the released binnings
were based; its table ends at the binnings of that time ($p_\pi$ five bins, $\theta_{\mu\pi}$
seven, $\cos\theta_\pi$ five), which were subsequently coarsened and, for $\cos\theta_\pi$,
re-binned on the current release (\S\ref{sec:binning_recheck}). The released binnings are
those of the analysis note's appendix.
...
re-tested on the 2026-09-04 release for whether finer or more bins are admissible. Two
...
\begin{center}\small
\captionof{table}{(Binning re-check on the 2026-09-04 release: ``released'' means that release; the forward-split five-bin $\cos\theta_\pi$ row is the binning released since 2026-09-06.) Pre-screen: smallest column-normalised migration diagonal over the true
...
\begin{center}\small
\captionof{table}{(Binning re-check on the 2026-09-04 release: ``released'' means that release; the forward-split five-bin $\cos\theta_\pi$ row is the binning released since 2026-09-06.) Full extraction of the candidates that passed the pre-screen, FHC / RHC:
...
\noindent\emph{Provenance.} This section records the 2026-08 pass in which the multi-run
overlay was assembled; its flux-term numbers and its description of the unfolded flux term
as ``amplified by the Wiener--SVD propagation'' are of that pass. The analysis note
(\noteref{Sec.}{sec:fluxanatomy}) now attributes the $17\%\to30\%$ growth to the correlated
background subtraction, $\delta\Phi/\Phi\,(1+B/S)$, demonstrated by the one-bin total; the
operator-amplification reading is superseded.
...
the flux-anatomy section of that pass attributed the elevated \emph{unfolded} flux to
that $\sim\!17\%$ model uncertainty amplified by the Wiener--SVD propagation through a
statistically-limited response, and that the limitation is the \emph{data}
statistics, not the overlay (the reading the analysis note has since superseded: the factor is $1+B/S$ from the correlated background subtraction, \noteref{Sec.}{sec:fluxanatomy}). Increasing the overlay alone therefore
...
\noindent\emph{Provenance.} This section records the 2026-08-28 pass on which the
combined-configuration defect was found and fixed. Its yields and the ``no detVar
systematic'' remarks about the proton-tagged family are of that pass; since 2026-08-31 all
proton-tagged extractions carry detector variations and the combined $p_\mu$ detector term
is $21.4\%$ (analysis note).
...
$A_C$, which differs per configuration and per observable. The observable-to-observable spread within a single configuration was, on the 2026-08-28
pass to which all percentages of this section refer, $15.9\%$ (FHC5), $17.4\%$
...
The detector systematic was anomalously large for COMB on that pass. For $p_\mu$ it was $23.1\%$ of
...
\begin{table}[H]
  ...
  \caption{Detector systematic \texttt{detVar\_total} as a percentage of the prediction,
    before and after pooling the detVar POT (2026-08-28 pass; the current detector terms
    are those of Tables~\ref{tab:systbreak_fhc}--\ref{tab:systbreak_comb}). A combined measurement should carry an
...
a \texttt{[IDENTITY]} line. Table~\ref{tab:identity} collects it for the $39$ extractions
of the pass on which it was compiled (six inclusive observables and seven proton-tagged ones
at their six-bin binnings, each in three beam configurations); the current release has $51$
(eleven proton-tagged observables, the TKI variables at two bins) and every one of them
...
\noindent The worst deviation over the $39$ extractions of that pass, and over the $51$ of
the current release, is $4.4\times10^{-6}$ relative, on FHC $W_{\pi p}$; the inclusive
...
% =============================================================================
\section{Data/MC normalisation: history of the corrections (superseded record)}
\label{sec:datamc}
% =============================================================================
...
does contain cosmics (custom pipeline only; the framework selects the treatment from the
presence of Monte Carlo truth and needs no switch).
...
\medskip\noindent\textbf{Detector variations.}  The detector systematic of the pass
described here ($9.6$--$14.6\%$; the current values are $15.6$--$35.0\%$,
\noteref{Table}{tab:systbreak}) was built from the Run-1 FHC detector variations applied
globally.  The multi-run FHC, RHC, and combined results instead
...
\noindent\emph{Provenance.} 2026-07 pass, on the GENIE detVar-CV and NuWro fake data of
that era; it predates the weighting fixes, the width-aware regularisation and the released
binnings and cannot be regenerated from the repository (analysis note, Limitations). Its
qualitative conclusions, no Wiener--SVD bias and NuWro/GENIE of order two, survive; none of
its numbers is current.
...
\begin{table}[H]
  \centering
  \caption{\textbf{[HISTORICAL, 2026-07; ratio column not trustworthy.]} NuWro fake-data
    closure: ratio of integrated cross sections and the $\chi^2$ of the unfolded NuWro
    result relative to the \textsc{Genie} tune. The ratio column was found on 2026-09-06 to
    be identical to the identity-regularised self-closure column of
    Table~\ref{tab:selfclose}, so it does not show the NuWro/\textsc{Genie} ratio of
    order two that the text and Fig.~\ref{fig:nuwro_closure} describe; the original values
    are not recoverable from the repository. The $\pmuM$ $\chi^2$ is undefined because its
    22-bin Wiener-SVD covariance is numerically singular (condition number
    $\sim4\times10^{13}$).}
...
\noindent\emph{Provenance.} Same 2026-07 pass as \S\ref{sec:nuwroclosure}; superseded
in its numbers (the current closure mean is $1.010$, analysis note).

The fake-data closures of that pass showed a consistent $\sim\!1.3\times$ ratio of the unfolded
