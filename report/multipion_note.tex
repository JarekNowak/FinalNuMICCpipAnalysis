...
\begin{titlepage}
  ...
  \begin{abstract}
    ...
    of $16.3\%$ ($6.4\%$) at an intrinsic purity of $21\%$ ($12\%$), corresponding to
    of order $530$ ($40$) selected signal events at full exposure. We present the
...
We also note a bookkeeping issue in the inherited signal definition that must be fixed for
the multi-pion channels: the momentum and opening-angle requirements are applied to
\texttt{TrueCandidatePionP}, which is assigned the \emph{last} charged pion encountered in
the daughter loop rather than a well-defined pion. For the single-pion signal this is
unambiguous, but for $N>1$ it applies the cut to an arbitrary pion. The fix, to be applied
together with the $0.10\GeVc$ threshold, is to require the softest of the $N$ pions to
exceed the threshold (equivalently, to apply the cut to all $N$).
...
With of order $530$ selected signal events at full exposure, the two-pion channel can
