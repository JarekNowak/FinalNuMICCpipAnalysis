// gen2d copy of costhpi5/neut_cc1pi_c5.C (2026-09-16): unchanged except for the added ext:: fills of obs_ext.h
...
// 5-bin theta_mu-pi edges; otherwise identical to ppi2bin/neut_cc1pi_2bin.C
...
void neut_incl(const char* infile, const char* outfile){
  TFile f(infile);
  ...
  for(long i=0;i<N;i++){ t->GetEntry(i);
    ...
    if(Ppi>0.175){ hpmu->Fill(Pmu);hcmu->Fill(pmu.CosTheta());hcpi->Fill(ppi.CosTheta());hth->Fill(th); X.fill_incl(pmu,ppi); } // others: standard 0.175
  }
  ...
  norm(hpmu);norm(hppi);norm(hcmu);norm(hcpi);norm(hth); for(size_t k=0;k<X.all.size();++k) norm(X.all[k]);
  TFile fo(outfile,"recreate"); hpmu->Write();hppi->Write();hcmu->Write();hcpi->Write();hth->Write(); for(size_t k=0;k<X.all.size();++k) X.all[k]->Write(); fo.Close();
