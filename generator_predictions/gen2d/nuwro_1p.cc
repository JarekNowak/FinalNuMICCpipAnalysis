// gen2d copy of analysis/nuwro_cc1pi1p.cc (2026-09-16): unchanged except for the added ext:: fills of obs_ext.h
...
int main(int argc,char**argv){
  ...
  for(TH1D* hh : X.all){ for(int b=1;b<=hh->GetNbinsX();b++){ double c=hh->GetBinContent(b),w=hh->GetBinWidth(b);
    hh->SetBinContent(b,sigma_tot*(c/(double)N)/w); hh->SetBinError(b,sigma_tot*(std::sqrt(c)/(double)N)/w); } }
  TFile fo(argv[2],"recreate"); for(int o=0;o<6;o++) h[o]->Write(); for(TH1D* hh : X.all) hh->Write(); fo.Close();
