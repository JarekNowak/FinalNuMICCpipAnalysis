// gen2d copy of costhpi5/analyze_gst_c5.C (2026-09-16): unchanged except for the added ext:: fills of obs_ext.h
// (proton-tagged theta_p / theta_pipr and the flattened 2D pairs). Existing histograms must stay bit-identical.
// costhpi5 (2026-09-06): forward-split cos(theta_pi) binning -1,-0.1,0.35,0.6,0.8,1 and the released
// 5-bin theta_mu-pi edges; otherwise identical to ppi2bin/analyze_gst_2bin.C
...
#include "obs_ext.h"
void analyze_gst_incl(const char* gstfile, double sigma_tot_cc_perAr, const char* outfile){
  ...
  std::vector<double> ECPI={-1.0,-0.10,0.35,0.60,0.80,1.0};
  std::vector<double> ETH={0.0,0.60,0.85,1.30,1.85,2.60};
  ...
  for(long i=0;i<N;i++){ t->GetEntry(i); if(!cc) continue;
    ...
    if(Ppi>0.175){ hpmu->Fill(Pmu);hcmu->Fill(pmu.CosTheta());hcpi->Fill(ppi.CosTheta());hth->Fill(th); X.fill_incl(pmu,ppi); } // others: standard 0.175 phase space
  }
  ...
  norm(hpmu);norm(hppi);norm(hcmu);norm(hcpi);norm(hth); for(auto* hh:X.all) norm(hh);
  TFile fo(outfile,"recreate"); hpmu->Write();hppi->Write();hcmu->Write();hcpi->Write();hth->Write(); for(auto* hh:X.all) hh->Write(); fo.Close();
