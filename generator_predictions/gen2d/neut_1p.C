// gen2d copy of newg4/neut/neut_cc1pi1p.C (2026-09-16): unchanged except for the added ext:: fills of obs_ext.h
...
void neut_1p(const char* infile, const char* outfile){
  ...
  for(size_t k=0;k<X.all.size();++k){ TH1D* hh=X.all[k]; for(int b=1;b<=hh->GetNbinsX();b++){ double c=hh->GetBinContent(b),w=hh->GetBinWidth(b);
    hh->SetBinContent(b,sperNuc*(c/(double)N)/w); hh->SetBinError(b,sperNuc*(std::sqrt(c)/(double)N)/w); } }
  TFile fo(outfile,"recreate"); for(int o=0;o<6;o++) h[o]->Write(); for(size_t k=0;k<X.all.size();++k) X.all[k]->Write(); fo.Close();
