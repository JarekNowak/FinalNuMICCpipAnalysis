// gen2d copy of newg4/gibuu/gibuu_cc1pi_1p.C (2026-09-16): unchanged except for the added ext:: fills of obs_ext.h
...
void gibuu_1p(const char* infile, double num_runs, const char* outfile){
  ...
  auto finish_event=[&](){
    ...
    if(n_mu==1&&n_pipm==1&&n_pi0==0&&n_kaon==0&&n_heavy==0 && leadp>0.3){
      ...
      if(pmu>0.15&&ppi>0.175&&th<2.6){
        ...
        X.fill_1p(p_mu,p_pi,p_pr,w_evt);
      }
    }
  };
  ...
  for(int o=0;o<6;o++) norm(h[o]); for(auto* hh:X.all) norm(hh);
  TFile fo(outfile,"recreate"); for(int o=0;o<6;o++) h[o]->Write(); for(auto* hh:X.all) hh->Write(); fo.Close();
