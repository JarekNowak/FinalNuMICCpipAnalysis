...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   TH1D *h_uncontained_muons_only = new TH1D("h_uncontained_muons_only", "Momentum Resolution for Uncontained Muons; Momentum Resolution ; N Particles",50,-5,5);
   TH1D *h_contained_muons_only = new TH1D("h_contained_muons_only", "Momentum Resolution for Contained Muons; Momentum Resolution  ; N Particles",100,-1.5,1.5);
   ...
   TH1D *h_opening_angle = new TH1D("h_opening_angle", "Muon Pion angle resolution; Resolution ; N Particles",100,-1,1);
   ...
   double muon_muon_contained = 0.0;
   double muon_pion_contained = 0.0;
   double muon_proton_contained = 0.0;
   double muon_other_contained = 0.0;
   double muon_and_pion = 0.0;
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
	...
	muon_total++;
	 ...
	 if((abs(backtracked_pdg->at(CC1mu1piXp_CandidateMuonIndex)) == 13)){//&& (CC1mu1piXp_CandidateMuonTrackEndContainment)){
...
/*	else if(abs(backtracked_pdg->at(CC1mu1piXp_CandidateMuonIndex)) == 211){// && (CC1mu1piXp_CandidateMuonTrackEndContainment)){
...
*/
	...
	if((CC1mu1piXp_CandidateMuonIndex != -1) && (CC1mu1piXp_CandidatePionIndex != -1)){

		if(abs(backtracked_pdg->at(CC1mu1piXp_CandidateMuonIndex)) == 211){

			muon_and_pion++;

		}	
		if((abs(backtracked_pdg->at(CC1mu1piXp_CandidateMuonIndex)) == 13) && (abs(backtracked_pdg->at(CC1mu1piXp_CandidatePionIndex)) == 211)){

		//muon_and_pion++;

		h_opening_angle->Fill(((mu_pi_opening_angle) - (true_mu_pi_opening_angle))/(true_mu_pi_opening_angle));
...
//	}
	 ...
	 std::cout<<"This is muon and pion "<< muon_and_pion <<std::endl;
	 ...
	 std::cout<<"Muon is a muon contained "<< muon_muon_contained <<std::endl;
         std::cout<<"Muon is a pion contained "<< muon_pion_contained << std::endl;
         std::cout<<"Muon is a proton contained  "<< muon_proton_contained << std::endl;
         std::cout<<"Muon is something else contained "<< muon_other_contained << std::endl;
...
*/
auto u = new TCanvas();
h_opening_angle->SetLineWidth(4);
h_opening_angle->Scale((2.0*pow(10,20))/(2.33*pow(10,21)));
h_opening_angle->SetLineColor(kRed);
