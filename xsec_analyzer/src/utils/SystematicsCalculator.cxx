...
std::cout<<"We do get in here but something goes wrong "<<std::endl;
...
void SystematicsCalculator::build_universes( TDirectoryFile& root_tdir ) {
  ...
  //std::cout<<"sys test 11.1 "<<std::endl;
  ...
  for ( const auto& run_and_type_pair : fpm.ntuple_file_map() ) {
   ...
   //std::cout<<"sys test 11.2 "<<std::endl;
	...
	//std::cout<<"sys test 11.3 "<<std::endl;
	...
	//std::cout<<"sys test 11.4 "<<std::endl;
  ...
  for ( const auto& run_and_type_pair : fpm.ntuple_file_map() ) {
	...
	//std::cout<<"sys test 11.6 "<<std::endl;
	...
	std::cout<<"sys test after processing universes 1 "<< std::endl;

	//std::cout<<"sys test 11.7 "<<std::endl;
	...
	//std::cout<<"sys test 11.8 "<<std::endl;

          if ( !temp_pot ) throw std::runtime_error(
            "Missing POT in MC file!" );
          file_pot = temp_pot->GetVal();

	 std::cout<<"sys test after processing universes 2 "<< std::endl;
	 ...
	 std::cout<<"sys test after processing universes 3 "<< std::endl;
        ...
        // current ntuple file
        //
        //std::cout<<"sys test 11.9 "<<std::endl;
         std::cout<<"sys test after processing universes 4 "<< std::endl;
	 ...
	 std::cout<<"sys test after processing universes 5 "<< std::endl;
	...
	//std::cout<<"sys test 12 "<<std::endl;
	 std::cout<<"sys test after processing universes 6 "<< std::endl;
          ...
          //std::cout<<"sys test 13 "<<std::endl;
           std::cout<<"sys test after processing universes 7 "<< std::endl;
  	 ...
  	 std::cout<<"sys test after processing universes 8 "<< std::endl;
	 ...
	 std::cout<<"sys test after processing universes 9 "<< std::endl;
	 ...
	 std::cout<<"sys test after processing universes 10 "<< std::endl;
	 ...
	 std::cout<<"sys test after processing universes 11 "<< std::endl;
	...
	std::cout<<"we get here "<<std::endl;
        ...
        // x as the true axis and y as the reco axis.
         std::cout<<"sys test after processing universes 12 "<< std::endl;

	if (!temp_2d_hist) {
  // This used to `return`, which exited build_universes() entirely from three
  // loops deep (run -> type -> file). The caller does not check for failure:
  // it would go on to save_universes(), writing half-filled histograms into
  // the input ROOT file, and every later run would silently reuse them via
  // load_universes(). The result was a CV prediction missing every run after
  // the bad file, with no diagnostic. A missing universe histogram means the
  // input is broken, so refuse outright rather than continuing with partial
  // POT.
  throw std::runtime_error( "Could not load histogram unweighted_0_2d for"
    " file " + file_name + ". The universe file is missing or corrupt;"
    " refusing to build partial universes." );
}
        ...
        if ( is_fake_data ) {
		 ...
		 std::cout<<"sys test after processing universes 13 "<< std::endl;


		std::cout<<"we get to fake data MC "<<std::endl;
	...
	//std::cout<<"sys test 14 "<<std::endl;
	 ...
	 std::cout<<"sys test after processing universes 15 "<< std::endl;
...
std::cout<<"Get covariances statement 1 works"<<std::endl;
...
std::cout<<"This is the name "<<name<<"This is the type "<<type<<std::endl;
	...
	std::cout<<"Get covariances statement 2 works"<<std::endl;
	...
	std::cout<<"Get covariances statement 3 works"<<std::endl;
	...
	std::cout<<"Get covariances statement 4 works"<<std::endl;
	...
	std::cout<<"Get covariances statement 5 works"<<std::endl;
	...
	std::cout<<"Get covariances statement 6 works"<<std::endl;
...
std::cout<<"Get covariances statement 7 works"<<std::endl;
	...
	std::cout<<"Get covariances statement 8 works"<<std::endl;
      ...
      make_cov_mat( *this, temp_cov_mat, cv_univ, alt_univ_vec,
        true, false );
	std::cout<<"Get covariances statement 9 works"<<std::endl;
    ...
    if ( matrix_map.count(name) ) {
      throw std::runtime_error( "Duplicate covariance matrix definition for "
        + name );
std::cout<<"Get covariances statement 10 works"<<std::endl;
...
std::cout<<"Get covariances statement 11 works"<<std::endl;
