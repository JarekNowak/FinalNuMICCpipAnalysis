  ...
  const TMatrixD& prior_true_signal ) const
{
  ...
  for ( int t = 0; t < num_true_signal_bins; ++t ) {
    ...
    // A zero (or numerically negligible) singular value means the response
    // matrix is rank deficient -- e.g. a true signal bin with no selected MC
    // events gives a zero column in smearcept. Without this guard W_C(t,t) is
    // also 0 and 0/0 = NaN, which then propagates through the matrix products
    // below and NaNs *every* bin of the unfolded result and its covariance,
    // not just the empty one.
    if ( dC * dC <= std::numeric_limits< double >::min() ) {
      W_C_tilde( t, t ) = 0.;
      std::cerr << "WARNING: WienerSVDUnfolder encountered a zero singular"
        " value for true bin " << t << ". The response matrix is rank"
        " deficient (most likely an empty true signal bin). Zeroing the"
        " corresponding Wiener filter element.\n";
      continue;
    }
    W_C_tilde( t, t ) /= dC * dC;
  }
