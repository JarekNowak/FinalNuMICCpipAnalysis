  ...
  while ( true ) {
    ...
    for ( int t = 0; t < num_true_signal_bins; ++t ) {
      for ( int r = 0; r < num_ordinary_reco_bins; ++r ) {
        ...
        // Guard both denominators. efficiency is 0 for a true bin with no
        // selected MC events; reco_expected is 0 when every true bin feeding
        // this reco bin is empty. Either gives Inf/NaN here, which propagates
        // into true_signal on this iteration and then into reco_expected on
        // the next, NaN-ing the entire measurement from iteration 2 onwards.
        // The analogous ots <= 0 case further down is already guarded this way.
        double denom = efficiency * reco_expected_element;
        double unfold_mat_element = 0.;
        if ( denom > 0. ) {
          unfold_mat_element = ( smearcept_element * true_signal_element )
            / denom;
        }
