...
#include "XSecAnalyzer/Selections/EventCategoriesXp_1.hh"
...
void CC1mu1p0pi::compute_reco_observables( AnalysisEvent* Event ) {
    ...
    && CandidateProtonIndex != BOGUS_INDEX)
  {
    ...
    TVector3 TVector3CandidateProton( -1, -1, -1 );
     ...
     CandidateMuMom = Event->track_range_mom_mu_->at(CandidateMuonIndex);
...
void CC1mu1p0pi::compute_true_observables( AnalysisEvent* Event ) {
  ...
  if ( isInteresting ) {
    ...
    TVector3 Muon_TVector3True( Muon_MCParticlePx, Muon_MCParticlePy,
      Muon_MCParticlePz );
    Muon_TrueMomentum_GeV = Muon_TVector3True.Mag(); // GeV
...
void CC1mu1p0pi::define_output_branches() {
  ...
  set_branch( &Muon_TrueMomentum_GeV,"Muon_TrueMomentum_GeV");
  set_branch( &CandidateMuMom,"CandidateMuMom");
...
void CC1mu1p0pi::reset() {
  ...
  Muon_TrueMomentum_GeV = BOGUS;
  CandidateMuMom = BOGUS;
}
