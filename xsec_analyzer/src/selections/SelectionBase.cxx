...
if (mc_signal_) signal_counter++;
if (selected_) selected_counter++;
if (mc_signal_ && selected_) selected_signal_counter++;
if (event_category_ == 0) event_category_sig_counter++;
//std::cout << "signal counter " << signal_counter << "selected counter " << selected_counter << "selected signal counter " << selected_signal_counter << std::endl;
}

void SelectionBase::summary() {
  std::cout << selection_name_ << " has " << num_passed_events_
    << " events which passed\n";
std::cout << "signal counter " << signal_counter << "selected counter " << selected_counter << "selected signal counter " << selected_signal_counter << " events cat sig " << event_category_sig_counter << std::endl;
