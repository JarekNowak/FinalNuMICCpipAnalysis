...
class CrossSectionExtractor {
  ...
  public:
    ...
    // Read-only access to the owned SystematicsCalculator. Used by diagnostic
    // drivers (e.g. UnfolderNuMI) to plot the intermediate reco-space and
    // true-space quantities (selected spectrum, background, smearceptance,
    // efficiency) that feed the unfolding.
    const SystematicsCalculator& get_syst() const { return *syst_; }
...
double CrossSectionExtractor::conversion_factor() const {
  ...
  // For the NuMI CC1mu1piXp analysis the fiducial volume excludes the z dead
  // region (675.1-775.1 cm), matching the custom selection's inFV(). Subtract
  // that slab from the target count so the normalisation is consistent.
  // (BNB path keeps the full FV — no dead-region subtraction.)
