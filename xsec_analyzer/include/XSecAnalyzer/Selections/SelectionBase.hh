...
protected:
 ...
 inline void define_containment_FV( double XMin, double XMax, double YMin,
    double YMax, double ZMin, double ZMax )
  {
    fv_containment_ = { XMin, XMax, YMin, YMax, ZMin, ZMax };
    set_fv_containment_ = true;
  }

  inline FiducialVolume containment_FV() {
    if ( !set_fv_containment_ ) {
      std::cerr << "Reco Fiducial volume has not been defined"
        << " for selection:" << selection_name_ << '\n';
      throw;
    }
    return fv_containment_;
  }
...
private:
  ...
  FiducialVolume fv_containment_;
  bool set_fv_true_ = false;
  bool set_fv_reco_ = false;
  bool set_fv_containment_ = false;
...
int event_category_sig_counter = 0;
