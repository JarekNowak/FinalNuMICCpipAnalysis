  ...
  const TMatrixD& cov_matrix )
{
  ...
  // The norm/shape decomposition divides by N_T and N_T^2 below. N_T is the
  // sum of the prediction, which is not sign-constrained for an unfolded
  // result, so it can legitimately be zero (an all-empty block, or entries
  // that cancel). Dividing would put NaN/Inf into norm_, shape_ and mixed_,
  // which then land in unfolded_cov_matrix_map_ under total_blockwise_*.
  // CovMatUtils.hh already guards the analogous case this way.
  if ( N_T == 0. ) {
    shape_.Zero();
    mixed_.Zero();
    norm_.Zero();
    mixed_plus_shape_.Zero();
    std::cerr << "WARNING: NormShapeCovMatrix received a prediction summing to"
      " zero. Returning zeroed norm/shape/mixed covariance matrices.\n";
    return;
  }
