...
public:
  ...
  // Software-trigger flag (1 = passes). Defaults to 1 so events from input
  // files lacking the branch are not removed (fail-open, matching the custom
  // selection ccpi_selection.C).
  int swtrig_ = 1;
  ...
  //Bragg variables
  MyPointer< std::vector<float> > trk_bragg_p_v;
  MyPointer< std::vector<float> > trk_bragg_mu_v;
  MyPointer< std::vector<float> > trk_bragg_mip_v;
  // Pion-hypothesis Bragg likelihood, used for proton rejection on the pion
