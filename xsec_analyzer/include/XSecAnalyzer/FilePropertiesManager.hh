  ...
  public:
	...
	std::cout << "Loading default file properties from "
      ...
      while ( std::getline(in_file, temp_line) ) {
        ...
        // std::string::front() is UB on an empty string, so a blank separator
        // line -- a very natural edit in a hand-maintained config -- read out
        // of bounds. Check empty() first.
        if ( temp_line.empty() || temp_line.front() == '#' ) continue;
