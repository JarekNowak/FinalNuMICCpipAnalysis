  ...
  int reco_bin, std::string event_category, int flux_universe_index ) const
{
  ...
  // Count how many true bins actually match. If none do, every contribution
  // below is skipped and this function returns 0., which make_cov_mat() then
  // turns into an identically-zero covariance matrix with no diagnostic --
  // i.e. the requested systematic silently vanishes from the total. That is
  // never a legitimate outcome, so fail loudly instead. See the check after
  // the loop.
  size_t num_matched_true_bins = 0u;

  for ( size_t tb = 0u; tb < num_true_bins; ++tb ) {
    const auto& tbin = true_bins_.at( tb );

    if (tbin.signal_cuts_ != category_string) continue;
    ++num_matched_true_bins;
