...
echo "[wait] combined throw..."; for i in $(seq 1 90); do grep -q "COMB seed" ../logs/throw_comb.log 2>/dev/null && break; sleep 20; done
grep -E "COMB seed" ../logs/throw_comb.log
echo "[build] combined FHC+RHC univmakes..."
pids=()
for o in pmu ppi costhmu costhpi thmupi; do
  FPM=configs/file_properties_numi_comb.txt BIN_CONFIG=configs/ccpi_${o}_bin_config_opt.txt \
    OUT=$PROC/ccpi_COMB_${o}_univmake.root ./run_universe_maker.sh > "../logs/comb_${o}_univ.log" 2>&1 &
  pids+=($!)
done
...
echo "===== COMBINED FHC+RHC FLUX BREAKDOWN + CLOSURE ====="
for o in pmu ppi costhmu costhpi thmupi; do
  fb=$(/tmp/flux_breakdown configs/ccpi_xsec_config_numi_${o}_comb.txt "$o" 2>/dev/null | grep "flux RECO")
  cl=$(bin/UnfolderNuMI configs/ccpi_xsec_config_numi_${o}_comb.txt configs/ccpi_${o}_slice_config_opt.txt /tmp/comb_${o}.root 2>/dev/null | grep -iE "^truth: " | head -1)
