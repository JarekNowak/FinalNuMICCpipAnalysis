// sb_plots.C -- the control-region distributions the collaboration will see at unblinding,
...
void sb_plots(const char* mode="fhc"){
  ...
  TCanvas c("c","",1800,1200); c.Divide(4,3); int pad=0;
  ...
  for(auto& v: vars){
    ...
    // The temporary must live in gDirectory: TTree::Draw(">>name") fills an EXISTING
    // histogram only if it finds it there, otherwise it silently books a new one.
    ...
    TLegend* lg=new TLegend(0.45,0.62,0.89,0.88); lg->SetBorderSize(0); lg->SetFillStyle(0); lg->SetTextSize(0.038); keep.push_back(lg);
    lg->SetHeader(Form("%s  %s", mode, v.reg+3)); lg->AddEntry(hdat,"full-stack pseudo-data","lep"); lg->AddEntry(hsig,"signal (contamination)","f"); lg->AddEntry(hbkg,"#nu background","f"); lg->AddEntry(hext,"beam-off","f"); lg->AddEntry(hdirt,"dirt","f"); lg->Draw();
  }
  c.SaveAs(Form("../report/figures/sideband_plots_%s.pdf",mode)); printf("wrote sideband_plots_%s.pdf\n",mode);
}
