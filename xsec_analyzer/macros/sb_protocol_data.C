// sb_protocol_data.C -- the statistics of the frozen control-region procedure applied to the
// REAL beam-on data of the signal-region-stripped skim. Run 2 is dropped from data AND
// prediction (no Run-2 beam-on file exists); no Poisson EXT/dirt is added to the data.
...
void sb_protocol_data(){
  ...
  for(int k=0;k<NK;k++){ Nd[k]=dataT[k]; pred[k]=cvT[k]+extT[k]+dirtT[k]; }
  for(int m=0;m<NM;m++) for(int i=0;i<NB;i++){ dH[m*NB+i]=dataH[m][i]; pH[m*NB+i]=cvH[m][i]+extH[m][i]+dirtH[m][i]; }
  // ---- covariance builders
  ...
  printf("\n==== CONTROL-REGION PROTOCOL STATISTICS on the BEAM-ON DATA (FHC runs 1,4,5; RHC runs 1,3,4; no Run 2) ====\n");
