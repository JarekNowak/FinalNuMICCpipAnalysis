...
static void add_ext(std::vector<Src>& v, const char* m){
  ...
  if(std::string(m)=="fhc"){ v.push_back({std::string(E)+R1,OCCX*9846635./G1}); v.push_back({std::string(E)+R1,OCCX*3535129./G1}); for(auto f:R4) v.push_back({std::string(E)+f,OCCX*4131149./G4}); v.push_back({std::string(E)+R5,OCCX*5154196./G5}); }
