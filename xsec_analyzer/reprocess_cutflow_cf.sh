#!/usr/bin/env bash
# Reprocess the 14 numuMC overlay files AND the dirt overlay into a SEPARATE directory
...
OUT=/data/uboone/processed/cf; mkdir -p "$OUT"
LOG=../logs/reprocess_cf_w; mkdir -p "$LOG"
...
proc_one() {
  ...
  nice -n 12 ProcessNTuples "$in" numuMC CC1mu1piXp "$out" > "$LOG/$n.log" 2>&1
  local rc=$?
